---------------------------------------------------
--          Z O N E S        M O D U L E         --
---------------------------------------------------

_.Zones =
{
	m(905, { 	-- Argus
		m(830, {	-- Krokuun
			n(-34, {	-- World Quests
				q(48285, {	-- Aerial Superiority
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(49043, {	-- Baneglow
					["lvl"] = 110,
					["provider"] = { "n", 128009 },	-- Baneglow
					["filterID"] = 101,	-- Battle Pet
					["isWorldQuest"] = true,
				}),
				q(47724, {	-- Besieged
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(47496, {	-- Bunker Buster
					["lvl"] = 110,
					["provider"] = { "n", 124015 },	-- Quartermistress Kl'azz
					["isWorldQuest"] = true,
				}),
				q(47624, {	-- Bury It
					["lvl"] = 110,
					["provider"] = { "n", 124452 },	-- Zul'cur the Ravenous
					["isWorldQuest"] = true,
				}),
				q(48511, {	-- Commander Endaxis
					["lvl"] = 110,
					["provider"] = { "n", 124775 },	-- Commander Endaxis
					["isWorldQuest"] = true,
				}),
				q(48509, {	-- Commander Sathrenael
					["lvl"] = 110,
					["provider"] = { "n", 122912 },	-- Commander Sathrenael
					["isWorldQuest"] = true,
				}),
				q(48510, {	-- Commander Vecaya
					["lvl"] = 110,
					["provider"] = { "n", 122911 },	-- Commander Vecaya
					["isWorldQuest"] = true,
				}),
				q(48286, {	-- Crystal Methods
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(49045, {	-- Deathscreech
					["lvl"] = 110,
					["provider"] = { "n", 128011 },	-- Deathscreech
					["filterID"] = 101,	-- Battle Pet
					["isWorldQuest"] = true,
				}),
				q(47720, {	-- Eternal Vengeance
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(49042, {	-- Foulclaw
					["isWorldQuest"] = true,
					["filterID"] = 101,	-- Battle Pet
					["lvl"] = 110,
				}),
				q(49046, {	-- Gnasher
					["isWorldQuest"] = true,
					["filterID"] = 101,	-- Battle Pet
					["lvl"] = 110,
				}),
				q(48287, {	-- Harvesters of Sorrow
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(48592, {	-- Holding the Spire
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(48282, {	-- Imp Mother Laglath
					["lvl"] = 110,
					["provider"] = { "n", 125820 },	-- Imp Mother Laglath
					["isWorldQuest"] = true,
				}),
				q(47507, {	-- Khazaduum
					["lvl"] = 110,
					["provider"] = { "n", 125824 },	-- Khazaduum
					["isWorldQuest"] = true,
				}),
				q(47551, {	-- Lessons of Fire and Pain
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(47707, {	-- Melting Down
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(48502, {	-- Naroua, King of the Forest
					["lvl"] = 110,
					["provider"] = { "n", 126419 },	-- Naroua, King of the Forest
					["isWorldQuest"] = true,
				}),
				q(48284, {	-- Reap the Fields
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(49044, {	-- Retch
					["filterID"] = 101,	-- Battle Pet
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(47646, {	-- Rope Around
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(49041, {	-- Ruinhoof
					["lvl"] = 110,
					["provider"] = { "n", 128007 },	-- Ruinhoof
					["filterID"] = 101,	-- Battle Pet
					["isWorldQuest"] = true,
				}),
				q(47542, {	-- Siegemaster Voraan
					["lvl"] = 110,
					["provider"] = { "n", 120393 },	-- Siegemaster Voraan
					["isWorldQuest"] = true,
				}),
				q(48512, {	-- Sister Subversia
					["lvl"] = 110,
					["provider"] = { "n", 123464 },	-- Sister Subversia <Coven of Shivarra>
					["isWorldQuest"] = true,
				}),
				q(47563, {	-- Spirits of the Fallen
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(48338, {	-- Supplies Needed: Astral Glory
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(47728, {	-- Talestra the Vile
					["lvl"] = 110,
					["provider"] = { "n", 123689 },	-- Talestra the Vile
					["isWorldQuest"] = true,
				}),
				q(48192, {	-- Tar Spitter
					["lvl"] = 110,
					["provider"] = { "n", 125479 },	-- Tar Spitter
					["isWorldQuest"] = true,
				}),
				q(47953, {	-- Terek the Selector
					["lvl"] = 110,
					["provider"] = { "n", 124804 },	-- Tereck the Selector
					["isWorldQuest"] = true,
				}),
				q(47712, {	-- The Devastator
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(47625, {	-- The Ritual We Share
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(48091, {	-- Vagath the Betrayed
					["lvl"] = 110,
					["provider"] = { "n", 125388 },	-- Vagath the Betrayed
					["isWorldQuest"] = true,
				}),
				q(48615, {	-- War Never Changes
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(48614, {	-- Woeful Implications
					["lvl"] = 110,
					["isWorldQuest"] = true,
				}),
				q(48337, {	-- Work Order: Astral Glory
					["lvl"] = 110,
					["isWorldQuest"] = true,
					["requireSkill"] = HERBALISM,
				}),
				q(48318, {	-- Work Order: Lightblood Elixirs
					["lvl"] = 110,
					["isWorldQuest"] = true,
					["requireSkill"] = ALCHEMY,
					["g"] = {
						i(151658),	-- Recipe: Lightblood Elixir (Rank 2)
					},
				}),
				q(48323, {	-- Work Order: Tears of the Naaru
					["requireSkill"] = ALCHEMY,
					["isWorldQuest"] = true,
					["lvl"] = 110,
					["g"] = {
						i(151704),	-- Recipe: Tears of the Naaru (Rank 2)
					},
				}),
			}),
		}),
	}),
};
