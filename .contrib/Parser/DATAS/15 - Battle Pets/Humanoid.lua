---------------------------------------------
--   P E T   J O U R N A L   M O D U L E   --
---------------------------------------------

_.BattlePets =
{
	n(-797,	{	-- Journal
		pa(238, {	-- Humanoid
			p(2755),	-- Adventurous Hopling
			p(1918, {	-- Alliance Enthusiast (Alliance)
				["races"] = ALLIANCE_ONLY,
			}),
			p(1155),	-- Anubisath Idol
			p(216),		-- Argent Gruntling
			p(214),		-- Argent Squire
			p(1323),	-- Ashleaf Spriteling
			p(1828, {	-- Baby Winston
				["u"] = REMOVED_FROM_GAME,
			}),
			p(1343),	-- Bonkers
			p(2457),	-- Coldlight Surfrunner
			p(1149),	-- Corefire Imp
			p(2899),	-- Court Messenger
			p(1688),	-- Crusher
			p(225, {	-- Curious Oracle Hatchling (A)
				["races"] = ALLIANCE_ONLY,
				["u"] = 20,	-- Children's Week
			}),
			p(226, {	-- Curious Oracle Hatchling (H)
				["races"] = HORDE_ONLY,
				["u"] = 20,	-- Children's Week
			}),
			p(2850),	-- Cursed Dune Watcher
			p(2900),	-- Dal
			p(1329),	-- Dandelion Frolicker
			p(294, {	-- Deathy
				["u"] = REMOVED_FROM_GAME,
			}),
			p(2967),	-- Dredger Butler
			p(1979, {	-- Dutiful Gruntling
				["races"] = HORDE_ONLY,
			}),
			p(1978, {	-- Dutiful Squire
				["races"] = ALLIANCE_ONLY,
			}),
			p(119),		-- Father Winter's Helper
			p(821),		-- Feral Vermling
			p(1229),	-- Fiendish Imp
			p(2778, {	-- Finduin (Blizzcon 2019)
				["races"] = ALLIANCE_ONLY,
				["u"] = REMOVED_FROM_GAME,
			}),
			p(514),		-- Flayer Youngling
			p(3130),	-- Gilded Darknight
			p(2777, {	-- Gillvanas (Blizzcon 2019)
				["races"] = HORDE_ONLY,
				["u"] = REMOVED_FROM_GAME,
			}),
			p(2888),	-- Glimr
			p(2114),	-- Globe Yeti
			p(333, {	-- Gregarious Grell
				["u"] = 10,	-- TCG
			}),
			p(1602, {	-- Grommloc
				["u"] = REMOVED_FROM_GAME,
			}),
			p(1725),	-- Grumpling
			p(228, {	-- Grunty
				["u"] = REMOVED_FROM_GAME,
			}),
			p(282, {	-- Guild Herald (Alliance)
				["altSpeciesID"] = 283,	-- Guild Herald (Horde)
				["races"] = ALLIANCE_ONLY,
			}),
			p(283, {	-- Guild Herald (Horde)
				["altSpeciesID"] = 282,	-- Guild Herald (Alliance)
				["races"] = HORDE_ONLY,
			}),
			p(280, {	-- Guild Page (Alliance)
				["altSpeciesID"] = 281,	-- Guild Page (Horde)
				["races"] = ALLIANCE_ONLY,
			}),
			p(281, {	-- Guild Page (Horde)
				["altSpeciesID"] = 280,	-- Guild Page (Alliance)
				["races"] = HORDE_ONLY,
			}),
			p(121, {	-- Gurky
				["u"] = REMOVED_FROM_GAME,
			}),
			p(1147),	-- Harbinger of Flame
			p(1157),	-- Harpy Youngling
			p(1384),	-- Hogs
			p(835),		-- Hopling
			p(1919, {	-- Horde Fanatic
				["races"] = HORDE_ONLY,
			}),
			p(2845),	-- K'uddly
			p(2683),	-- Kelpfin
			p(1940, {	-- Knight-Captain Murky
				["races"] = ALLIANCE_ONLY,
				["u"] = REMOVED_FROM_GAME,
			}),
			p(1166),	-- Kun-Lai Runt
			p(1939, {	-- Legionnaire Murky
				["races"] = HORDE_ONLY,
				["u"] = REMOVED_FROM_GAME,
			}),
			p(1226),	-- Lil' Bad Wolf
			p(3062),	-- Lost Featherling
			p(111, {	-- Lurky
				["u"] = REMOVED_FROM_GAME,
			}),
			p(3131),	-- Mawsworn Minion
			p(189, {	-- Mini Tyrael
				["u"] = REMOVED_FROM_GAME,
			}),
			p(296, {	-- Moonkin Hatchling (Alliance)
				["races"] = ALLIANCE_ONLY,
				["u"] = REMOVED_FROM_GAME,
			}),
			p(298, {	-- Moonkin Hatchling (Horde)
				["races"] = HORDE_ONLY,
				["u"] = REMOVED_FROM_GAME,
			}),
			p(3132),	-- Mord'al Eveningstar
			p(2681),	-- Murgle
			p(329, {	-- Murkablo
				["u"] = REMOVED_FROM_GAME,
			}),
			p(1364, {	-- Murkalot
				["u"] = REMOVED_FROM_GAME,
			}),
			p(1168, {	-- Murki
				["u"] = REMOVED_FROM_GAME,
			}),
			p(1454, {	-- Murkidan
				["u"] = REMOVED_FROM_GAME,
			}),
			p(217, {	-- Murkimus the Gladiator
				["u"] = REMOVED_FROM_GAME,
			}),
			p(107, {	-- Murky
				["u"] = REMOVED_FROM_GAME,
			}),
			p(2682),	-- Necrofin Tadpole
			p(1495),	-- Ore Eater
			p(248, {	-- Pandaren Monk
				["u"] = BLIZZARD_BALANCE,
			}),
			p(122),		-- Peddlefeet
			p(513),		-- Qiraji Guardling
			p(2417),	-- Ranishu Runt
			p(2135),	-- Rebellious Imp
			p(3133),	-- Rook
			p(1349),	-- Rotten Little Helper
			p(3042),	-- Ruffle
			p(1628),	-- Sister of Temptation
			p(2762),	-- Slimy Darkhunter
			p(1953),	-- Snobold Runt
			p(515),		-- Sporeling Sprout
			p(1935),	-- Squirky
			p(3067),	-- Spriggan Trickster
			p(1935),	-- Squirky
			p(3061),	-- Steward Featherling
			p(1158),	-- Stunted Yeti
			p(1885),	-- Sunborne Val'kyr
			p(1073),	-- Terky
			p(2469),	-- Thistlebrush Bud
			p(1803),	-- Thistleleaf Adventurer
			p(2901),	-- Trapped Stonefiend
			p(1365, {	-- Treasure Goblin
				["u"] = REMOVED_FROM_GAME,
			}),
			p(2022),	-- Tylarr Gronnden
			p(2136),	-- Uuna
			p(2833),	-- Void-Scarred Anubisath
			p(2184, {	-- Whomper
				["u"] = BLIZZARD_BALANCE,
			}),
			p(120),		-- Winter's Little Helper
			p(3079),	-- Winterleaf Spriggan
			p(1634),	-- Wretched Servant
			p(1887),	-- Wyrmy Tunkins
			p(2680),	-- Zanj'ir Poker
		}),
	}),
};
