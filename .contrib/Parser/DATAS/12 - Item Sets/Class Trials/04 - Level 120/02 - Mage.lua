-----------------------------------------------------
--       G E A R     S E T S     M O D U L E       --
-----------------------------------------------------
_.GearSets = 
{
	n(-5350, {	-- Class Trial Item Sets
		n(-155, {	-- Level 120
			["groups"] = {
				n(-145, {	-- Mountainsage
					["groups"] = {
						{	-- Mountainsage Staff	*LA0207
							["itemID"] = 170646,	-- Mountainsage Stave
						},
						{	-- Mountainsage Hood	*LA0207
							["itemID"] = 170809,	-- Mountainsage Hood
						},
						un(1, i(170640)),	-- Mountainsage Medallion
						{	-- Mountainsage Shoulderpads	*LA0207
							["itemID"] = 170812,	-- Mountainsage Shoulderpads
						},
						{	-- Mountainsage Cloak of Destruction	*LA0207
							["itemID"] = 170645,	-- Mountainsage Cloak of Destruction
						},
						{	-- Mountainsage Robe	*LA0207
							["itemID"] = 170811,	-- Mountainsage Robe
						},
						{	-- Mountainsage Wristwraps	*LA0207
							["itemID"] = 170814,	-- Mountainsage Wristwraps
						},
						{	-- Mountainsage Handwraps	*LA0207
							["itemID"] = 170808,	-- Mountainsage Handwraps
						},
						{	-- Mountainsage Cord	*LA0207
							["itemID"] = 170813,	-- Mountainsage Cord
						},
						{	-- Mountainsage Leggings	*LA0207
							["itemID"] = 170810,	-- Mountainsage Leggings
						},
						{	-- Mountainsage Sandals	*LA0207
							["itemID"] = 170807,	-- Mountainsage Sandals
						},
						{	-- Mountainsage Band of Destruction	*LA0207
							["itemID"] = 170642,	-- Mountainsage Band of Destruction
						},
						{	-- Mountainsage Ring of Destruction	*LA0207
							["itemID"] = 170643,	-- Mountainsage Ring of Destruction
						},
						{	-- Mountainsage Stone of Destruction	*LA0207
							["itemID"] = 170644,	-- Mountainsage Stone of Destruction
						},
						{	-- Mountainsage Idol of Destruction	*LA0207
							["itemID"] = 170641,	-- Mountainsage Idol of Destruction
						},
					},
					["classes"] = { MAGE }
				}),
			},
		}),
	}),
};