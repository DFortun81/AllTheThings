    ...
    public static partial class Framework
    {
        ...
        public static void Export()
        {
            ...
            if (outputFolder.Exists)
            {
                ...
                // General ExportDBs -- now included in LocalizationDB
                foreach (var exportDB in Exports)
                {
                    //var exportFilename = Path.Combine(addonRootFolder, $"db/{dbRootFolder}{exportDB.Key}.lua");
                    var exportContent = ExportPureLua(exportDB.Value).Insert(0, $"_.{exportDB.Key}=\n").ToString();
                    localizationDatabase.AppendLine(exportContent);
                }
