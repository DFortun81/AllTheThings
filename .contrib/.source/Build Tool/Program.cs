...
namespace ATT
{
    ...
    class Program
    {
        ...
        /// <summary>
        /// The current build version. (This gets replaced.)
        /// </summary>
        static string CurrentBuildVersion = "[Git]";

        /// <summary>
        /// The last build version. (This gets replaced.)
        /// </summary>
        static string LastBuildVersion = "[Git]";
        ...
        static void Build(DirectoryInfo directory)
        {
            ...
            // Attempt to pull from Git and generate the current build version number.
            LastBuildVersion = GitPullTag();
            CurrentBuildVersion = CalculateNextVersion(LastBuildVersion);
            ...
            // Attempt to Tag the last commit with the version number to the Git
            GitTagVersion(CurrentBuildVersion);

            // Determine the name of the ZIP File.
            var ZipFileName = Path.Combine(directory.FullName, $".contrib/.builds/{directory.Name}-v{CurrentBuildVersion}.zip");
            ...
            using (var zipFile = ZipFile.Open(ZipFileName, ZipArchiveMode.Create))
            {
                ...
                {
                    writer.WriteLine(GitGatherChanges(LastBuildVersion, CurrentBuildVersion));
                }
            }
        ...
        static void GitTagVersion(string version)
        {
        ...
        /// <summary>
        /// Attempt to pull changes and the tag from the Git.
        /// </summary>
        /// <returns>The last tag.</returns>
        static string GitPullTag()
        {
            ...
            try
            {
                using (Process p = new Process())
                {
                    var startInfo = new ProcessStartInfo("git", "describe --abbrev=0 --tags")
                    {
                        UseShellExecute = false,
                        RedirectStandardOutput = true,
                        WorkingDirectory = Path.GetDirectoryName("../..")
                    ...
                    p.StartInfo = startInfo;
                    p.Start();
                    var tag = p.StandardOutput.ReadToEnd().Trim();
                    p.WaitForExit();
                    return tag;
                }
            }
