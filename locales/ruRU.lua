...
-- Settings.lua
	...
	-- General tab
		...
		L.FACTION_MODE = "Только Текущая Фракция";
		...
		L.HIDE_PVP_CHECKBOX = "Скрыть ПвП Контент";
		L.HIDE_PVP_CHECKBOX_TOOLTIP = "Включите данную опцию, если Вы хотите скрыть весь контент, который 'может' требовать ПвП.";
	...
	-- Interface tab
		...
		L.DROP_CHANCES_CHECKBOX = "Показывать Шанс Выпадения";
...
for key,value in pairs({
	...
	[12] = {0, "|CFF00FFDEЭтот предмет или достижение требует действий игрока против игрока или валюты, связанной с этими действиями.|r", "Требуется PvP"},
	
	-- Arbitrary Filters
	[9] = {3, "|CFFFF0000Первоначальный источник его получения был удален и теперь доступен только на аукционе черного рынка.|r", "Черный рынок AH [BMAH]"},
	[10] = {3, "|CFFFF0000Первоначально полученная через карту TCG, которая больше не печатается, но все еще может быть доступна на черном рынке, в игре или на аукционах в реальной жизни.|r", "Коллекционная карточная игра [TCG]"},
	...
	[13] = {1, "|CFFFF0000Ваши последователи слишком высоки, и миссия для тайника больше не будет появляться.|r", "Устаревший кеш"},
	[14] = {3, "|CFFFF0000Его больше нельзя будет купить, если у вас нет необходимого PvP-рейтинга или если вы не входили в% лучших в этом сезоне.|r", "ПвП Элита / Гладиатор"},
	[15] = {1, "|CFFFF0000Его нельзя выучить навсегда или использовать для трансмогрификации.|r", "Неизучаемые"},
	...
	-- Seasonal Filters
	[24] = {4, "|CFF00FFDEМожет требоваться действующий Хмельной Фестиваль, чтобы получить данный предмет|r", "Хмельной Фестиваль"},
	[20] = {4, "|CFF00FFDEМожет требоваться действующая Детская Неделя, чтобы получить данный предмет.|r", "Детская Неделя"},
	[1012] = {4, "|CFF00FFDEДля этого требуется, чтобы Праздник ярмарки Новолуния был активным.|r", "Ярмарка ярмарки Новолуния" },
	[27] = {4, "|CFF00FFDEМожет требоваться действующий День Мертвых, чтобы получить данный предмет.|r", "День Мертвых"},
	[29] = {4, "|CFF00FFDEМожет требоваться действующий Зимний Покров, чтобы получить данный предмет.|r", "Зимний Покров"},
	[26] = {4, "|CFF00FFDEМожет требоваться действующий Тыквовин, чтобы получить данный предмет.|r", "Тыквовин"},
	[25] = {4, "|CFF00FFDEМожет требоваться действующая Неделя Урожая, чтобы получить данный предмет.|r", "Неделя Урожая"},
	[18] = {4, "|CFF00FFDEМожет требоваться действующая Любовная Лихорадка, чтобы получить данный предмет.|r", "Любовная Лихорадка"},
	[17] = {4, "|CFF00FFDEМожет требоваться действующий Лунный Фестиваль, чтобы получить данный предмет.|r", "Лунный Фестиваль"},
	[22] = {4, "|CFF00FFDEДля этого требуется, чтобы был активен определенный микро-праздник.|r", "Маленькие праздники"},
	[21] = {4, "|CFF00FFDEМожет требоваться действующий Огненный Солнцеворот, чтобы получить данный предмет.|r", "Огненный Солнцеворот"},
	[1009] = {4, "|CFF00FFDEДля этого необходимо, чтобы праздник празднования Нового года был активным.|r", "New Years Celebration" },
	[19] = {4, "|CFF00FFDEМожет требоваться действующий Сад Чудес, чтобы получить данный предмет.|r", "Сад Чудес"},
	[28] = {4, "|CFF00FFDEМожет требоваться действующее Пиршество Странников, чтобы получить данный предмет.|r", "Пиршество Странников"},
	[23] = {4, "|CFF00FFDEДля этого должен быть Активен Праздник Пиратского Дня.|r", "День Пирата"},
	[30] = {4, "|CFF00FFDEМожет требоваться действующая Рыбомания Тернистой Долины.|r", "Рыбомания Тернистой Долины"},
	[42] = {4, "|CFFFF0000Для этого необходимо, чтобы событие путешествия во времени было активным.\nБудет доступен только контент подземелий, выпущенный во время активного расширения \"Путешествие во времени\".|r", "Путешествие во времени"},
	[31] = {4, "|CFF00FFDEМожно получить только во время действующей Годовщины WoW.\nЕсть вероятность, что последующие годовщины будут включать данный предмет.|r", "Годовщина WoW"},
})
do a[key] = value; end
