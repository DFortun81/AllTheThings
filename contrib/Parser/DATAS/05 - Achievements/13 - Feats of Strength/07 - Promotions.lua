--------------------------------------------------
--   A C H I E V E M E N T S    M O D U L E    --
--------------------------------------------------

_.Achievements =
{
	n(-9969, {	-- Feats of Strength
		["groups"] = {
			n(-10046, {	-- Promotions
				["groups"] = {
					un(16, ach(415)),		-- Big Blizzard Bear
					un(6, ach(11064)),		-- Collector's Edition: Baby Winston
					un(6, ach(7842)),		-- Collector's Edition: Baneling
					un(6, ach(8917)),		-- Collector's Edition: Dread Hatchling
					un(6, ach(8916)),		-- Collector's Edition: Dread Raven
					un(6, ach(7412)),		-- Collector's Edition: Fetish Shaman
					un(6, ach(683)),		-- Collector's Edition: Frost Wyrm Whelp
					{	-- Collector's Edition: Gilded Ravasaur // Collector's Edition: Seabraid Stallion
						["allianceAchievementID"] = 12229,	-- Collector's Edition: Seabraid Stallion
						["hordeAchievementID"] = 12230,		-- Collector's Edition: Gilded Ravasaur
					},
					un(6, ach(10320)),		-- Collector's Edition: Illidari Felstalker
					un(6, ach(6849)),		-- Collector's Edition: Imperial Quilen
					un(6, ach(5377)),		-- Collector's Edition: Lil' Deathwing
					un(6, ach(6848)),		-- Collector's Edition: Lucky Quilen Cub
					un(6, ach(4824)),		-- Collector's Edition: Mini Thor
					un(6, ach(662)),		-- Collector's Edition: Mini-Diablo
					un(6, ach(665)),		-- Collector's Edition: Netherwhelp
					un(6, ach(10321)),		-- Collector's Edition: Nibbles
					un(6, ach(663)),		-- Collector's Edition: Panda
					un(6, ach(12232)),		-- Collector's Edition: Tottle
					un(6, ach(8795)),		-- Collector's Edition: Treasure Goblin
					un(6, ach(10309)),		-- Collector's Edition: Zeradar
					un(6, ach(664)),		-- Collector's Edition: Zergling
					un(16, ach(5378)),		-- Deathy
					un(3, a(ach(11210, {	-- Fight for the Alliance
						i(140999),			-- Replica Lion's Fang
						i(141000),			-- Replica Lion's Heart
					}))),
					un(3, h(ach(11211, {	-- Fight for the Horde
						i(141001),			-- Replica Blood Guard's Cleaver
						i(141002),			-- Replica Staff of Gul'dan
					}))),
					ach(10657),				-- Fledgling Hero of Warcraft
					un(16, ach(9763)),		-- Grommloc
					ach(8345, {				-- Hearthstoned
						i(98618),			-- Hearthsteed
					}),
					un(3, ach(11425)),		-- Herald of Flames
					ach(9926),				-- Hero of the Storm
					ach(13196, {			-- Meat Marauder
						i(164571),			-- Meat Wagon
					}),
					un(16, ach(6185)),		-- Murkablo
					un(16, ach(8793)),		-- Murkalot
					un(16, ach(10322)),		-- Murkidan
					un(16, ach(411)),		-- Murky
					un(16, ach(11294)),		-- Murloc Battlemasters
					un(16, ach(412)),		-- Murloc Costume
					un(3, ach(10537)),		-- Patron of War
					un(16, ach(11931)),		-- Rides of War
					un(3, ach(12454)),		-- Salute to Starcraft
					un(16, ach(3536)),		-- The Marine Marine
					un(3, ach(414)),		-- Tyrael's Hilt
					un(3, h(ach(9496, { 	-- Warlord's Deathwheel
						i(116788),			-- Warlord's Deathwheel
					}))),
				},
			}),
		},
	}),
};