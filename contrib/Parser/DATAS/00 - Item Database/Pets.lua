...
model(59668, pet(117528, 1533));	-- Lanticore Spawnling
...
model(57823, pet(118599,  1429)); -- Autumnal Sproutling
...
model(64589, pet(128427, 1454));	-- Murkidan
...
model(68860, pet(140316, 1928));	-- Firebat Pup
model(68845, pet(140320, 1929));	-- Corgnelius
...
model(77501, pet(151234, 2062));	-- Shadow
model(74514, pet(151269, 2002));	-- Naxxy
pet(151569, 2063); 		-- Sneaky Marmot
pet(151632, 2064); 		-- Mining Monkey
pet(151633, 2065); 		-- Dig Rat
model(74513, pet(151645, 2001));	-- Dibbler
pet(151828, 2072); 		-- Ageless Bronze Drake
pet(151829, 2071); 		-- Bronze Proto-Whelp
model(78989, pet(152555, 2077));	-- Ghost Shark
model(79712, pet(152963, 2093));	-- Amalgam of Destruction
model(79630, pet(152966, 2078));	-- Tinytron
model(79634, pet(152967, 2079));	-- Discarded Experiment
model(24751, pet(152968, 2080));	-- Rattlejaw
model(79635, pet(152969, 2081));	-- Twilight Clutch-Sister
model(79637, pet(152970, 2082));	-- Bound Stream
model(79638, pet(152972, 2083));	-- Faceless Minion
model(79641, pet(152973, 2084));	-- Zephyrian Prince
model(79639, pet(152974, 2085));	-- Drafty
model(79707, pet(152975, 2086));	-- Blazehound
model(45289, pet(152976, 2087));	-- Cinderweb Recluse
model(79709, pet(152977, 2088));	-- Vibrating Stone (Surger)
model(38283, pet(152978, 2089));	-- Infernal Pyreclaw
model(65888, pet(152979, 2090));	-- Faceless Mindlasher
model(71972, pet(152980, 2091));	-- Corrupted Blood
model(79711, pet(152981, 2092));	-- Unstable Tendril
model(79777, pet(153026, 2115));	-- Cross Gazer
model(79776, pet(153027, 2116)); 	-- Orphaned Marsuul
model(78451, pet(153040, 2113));	-- Felclaw Marsuul
model(79783, pet(153045, 2117));	-- Fel Lasher
model(79796, pet(153054, 2118));	-- Docile Skyfin
model(79797, pet(153055, 2119));	-- Fel-Afflicted Skyfin
model(79760, pet(153056, 2120));	-- Grasping Manifestation
model(79798, pet(153057, 2121));	-- Fossorial Bile Larva
model(76829, pet(153195, 2136));	-- Uuna
model(79888, pet(153252, 2135));	-- Rebellious Imp
