---------------------------------------------------
--          Z O N E S        M O D U L E         --
---------------------------------------------------
_.Zones =
{
	{	-- Draenor
		["mapID"] = 572,	-- Draenor
		["g"] = {
			{	-- Nagrand (Draenor)
				["mapID"] = 550,	-- Nagrand (Draenor)
				["g"] = {
					{	-- Rares
						["npcID"] = -16, 	-- Rares
						["g"] = {
							-- Coords have been confirmed on rares except for Edge of Reality, Luk'hok, Nakk, and a few noted long spawns.
							{	-- Abandoned Cargo
								["objectID"] = 233206, 	-- Abandoned Cargo
								["questID"] = 35759,
								["coords"] = {
									{ 67.6, 59.8, 550 },
								},
								["g"] = {
									-- Weapons
									i(116627),	-- Howling Axe
									i(116629),	-- Howling Bow
									i(116628),	-- Howling Broadaxe
									i(116630),	-- Howling Crossbow
									i(116631),	-- Howling Dagger
									i(116642),	-- Howling Greatsword
									i(116634),	-- Howling Gun
									i(116637),	-- Howling Hammer
									i(116633),	-- Howling Knuckles
									i(116635),	-- Howling Mace
									i(116638),	-- Howling Spear
									i(116640),	-- Howling Staff
									i(116639),	-- Howling Staff
									i(116641),	-- Howling Sword
									i(116724),	-- Oshu'gun Basin
									i(116636),	-- Oshu'gun Scepter
									i(116632),	-- Oshu'gun Spellblade
									i(116643),	-- Oshu'gun Wand
									i(106727),	-- Stonecrag Shield
									-- Head
									i(106738),	-- Dark Star Cowl
									i(106733),	-- Meadowstomper Hood
									i(106749),	-- Packrunner Helm
									i(106723),	-- Stonecrag Helm
									-- Neck
									i(116687),	-- Oshu'gun Amulet
									-- Shoulders
									i(106743),	-- Dark Star Shoulderpads
									i(106735),	-- Meadowstomper Shoulders
									i(106751),	-- Packrunner Spaulders
									i(106725),	-- Stonecrag Pauldrons
									-- Back
									i(116723),	-- Oshu'gun Cloak
									-- Chest
									i(106741),	-- Dark Star Robe
									i(106734),	-- Meadowstomper Jerkin
									i(106752),	-- Packrunner Vest
									i(106720),	-- Stonecrag Breastplate
									-- Wrist
									i(106744),	-- Dark Star Wristwraps
									i(106729),	-- Meadowstomper Bindings
									i(106746),	-- Packrunner Bracers
									i(106728),	-- Stonecrag Vambraces
									-- Hands
									i(106739),	-- Dark Star Handwraps
									i(106732),	-- Meadowstomper Gloves
									i(106747),	-- Packrunner Gauntlets
									i(106721),	-- Stonecrag Gauntlets
									-- Waist
									i(106737),	-- Dark Star Cord
									i(106736),	-- Meadowstomper Waistband
									i(106745),	-- Packrunner Belt
									i(106722),	-- Stonecrag Girdle
									-- Legs
									i(106740),	-- Dark Star Leggings							
									i(106731),	-- Meadowstomper Britches
									i(106750),	-- Packrunner Legguards
									i(106724),	-- Stonecrag Legplates
									-- Feet
									i(106742),	-- Dark Star Sandals
									i(106730),	-- Meadowstomper Boots
									i(106748),	-- Packrunner Greaves
									i(106726),	-- Stonecrag Sabatons
									-- Finger
									i(116725),	-- Oshu'gun Ring
									i(116920, {	-- True Steel Lockbox
										-- Weapons
										i(116721),	-- Ancestral Branch
										i(116619),	-- Ancestral Scepter
										i(116626),	-- Ancestral Wand
										i(116613),	-- Gorian Crossbow
										i(116614),	-- Gorian Dagger
										i(116625),	-- Gorian Greatsword
										i(116617),	-- Gorian Gun
										i(116620),	-- Gorian Hammer
										i(116618),	-- Gorian Mace
										i(116621),	-- Gorian Spear
										i(116622),	-- Gorian Staff
										i(116627),	-- Howling Axe
										i(116634),	-- Howling Gun
										i(116633),	-- Howling Knuckles
										i(116640),	-- Howling Staff
										i(116632),	-- Oshu'gun Spellblade
										i(116643),	-- Oshu'gun Wand
										-- Head
										i(106690),	-- Ironfist Helm
										i(106723),	-- Stonecrag Helm
										-- Shoulders
										i(106692),	-- Ironfist Pauldrons
										i(106735),	-- Meadowstomper Shoulders
										i(106702),	-- Sabermaw Shoulders
										-- Chest
										i(106687),	-- Ironfist Breastplate
										i(106701),	-- Sabermaw Jerkin
										i(106720),	-- Stonecrag Breastplate
										i(106708),	-- Voidwrap Robe
										-- Wrist
										i(106729),	-- Meadowstomper Bindings
										i(106713),	-- Sunspring Bracers							
										i(106711),	-- Voidwrap Wristwraps
										-- Hands
										i(106747),	-- Packrunner Gauntlets
										i(106699),	-- Sabermaw Gloves
										i(106714),	-- Sunspring Gauntlets
										i(106706),	-- Voidwrap Handwraps
										-- Waist
										i(106722),	-- Stonecrag Girdle
										i(106712),	-- Sunspring Belt
										i(106704),	-- Voidwrap Cord
										-- Legs
										i(106740),	-- Dark Star Leggings
										i(106691),	-- Ironfist Legplates
										i(106717),	-- Sunspring Legguards
										-- Feet
										i(106697),	-- Sabermaw Boots
										i(106726),	-- Stonecrag Sabatons
									}),							
								},
							},
							{	-- Adventurer's Mace
								["objectID"] = 233650,	-- Adventurer's Mace
								["questID"] = 36077,
								["coords"] = {
									{ 75.8, 62.0, 550 },
								},
								["g"] = {
									i(116635),	-- Howling Mace
									i(116636), 	-- Oshu'gun Scepter
								},
							},
							{	-- Adventurer's Pack
								["objectID"] = 233218,	-- Adventurer's Pack
								["questID"] = 35765,
								["coords"] = {
									{ 82.27, 56.59, 550 },
								},
								["g"] = {
									-- Weapons
									i(116627),	-- Howling Axe
									i(116629),	-- Howling Bow
									i(116628),	-- Howling Broadaxe
									i(116630),	-- Howling Crossbow
									i(116631),	-- Howling Dagger
									i(116642),	-- Howling Greatsword
									i(116634),	-- Howling Gun
									i(116637),	-- Howling Hammer
									i(116633),	-- Howling Knuckles
									i(116635),	-- Howling Mace
									i(116638),	-- Howling Spear
									i(116640),	-- Howling Staff
									i(116639),	-- Howling Staff
									i(116641),	-- Howling Sword
									i(116724),	-- Oshu'gun Basin
									i(116636),	-- Oshu'gun Scepter
									i(116632),	-- Oshu'gun Spellblade
									i(116643),	-- Oshu'gun Wand
									i(106727),	-- Stonecrag Shield
									-- Head
									i(106738),	-- Dark Star Cowl
									i(106733),	-- Meadowstomper Hood
									i(106749),	-- Packrunner Helm
									i(106723),	-- Stonecrag Helm
									-- Neck
									i(116687),	-- Oshu'gun Amulet
									-- Shoulders
									i(106743),	-- Dark Star Shoulderpads
									i(106735),	-- Meadowstomper Shoulders
									i(106751),	-- Packrunner Spaulders
									i(106725),	-- Stonecrag Pauldrons
									-- Back
									i(116723),	-- Oshu'gun Cloak
									-- Chest
									i(106741),	-- Dark Star Robe
									i(106734),	-- Meadowstomper Jerkin
									i(106752),	-- Packrunner Vest
									i(106720),	-- Stonecrag Breastplate
									-- Wrist
									i(106744),	-- Dark Star Wristwraps
									i(106729),	-- Meadowstomper Bindings
									i(106746),	-- Packrunner Bracers
									i(106728),	-- Stonecrag Vambraces
									-- Hands
									i(106739),	-- Dark Star Handwraps
									i(106732),	-- Meadowstomper Gloves
									i(106747),	-- Packrunner Gauntlets
									i(106721),	-- Stonecrag Gauntlets
									-- Waist
									i(106737),	-- Dark Star Cord
									i(106736),	-- Meadowstomper Waistband
									i(106745),	-- Packrunner Belt
									i(106722),	-- Stonecrag Girdle
									-- Legs
									i(106740),	-- Dark Star Leggings							
									i(106731),	-- Meadowstomper Britches
									i(106750),	-- Packrunner Legguards
									i(106724),	-- Stonecrag Legplates
									-- Feet
									i(106742),	-- Dark Star Sandals
									i(106730),	-- Meadowstomper Boots
									i(106748),	-- Packrunner Greaves
									i(106726),	-- Stonecrag Sabatons
									-- Finger
									i(116725),	-- Oshu'gun Ring
									i(116920, {	-- True Steel Lockbox
										-- Weapons
										i(116721),	-- Ancestral Branch
										i(116619),	-- Ancestral Scepter
										i(116626),	-- Ancestral Wand
										i(116613),	-- Gorian Crossbow
										i(116614),	-- Gorian Dagger
										i(116625),	-- Gorian Greatsword
										i(116617),	-- Gorian Gun
										i(116620),	-- Gorian Hammer
										i(116618),	-- Gorian Mace
										i(116621),	-- Gorian Spear
										i(116622),	-- Gorian Staff
										i(116627),	-- Howling Axe
										i(116634),	-- Howling Gun
										i(116633),	-- Howling Knuckles
										i(116640),	-- Howling Staff
										i(116632),	-- Oshu'gun Spellblade
										i(116643),	-- Oshu'gun Wand
										-- Head
										i(106690),	-- Ironfist Helm
										i(106723),	-- Stonecrag Helm
										-- Shoulders
										i(106692),	-- Ironfist Pauldrons
										i(106735),	-- Meadowstomper Shoulders
										i(106702),	-- Sabermaw Shoulders
										-- Chest
										i(106687),	-- Ironfist Breastplate
										i(106701),	-- Sabermaw Jerkin
										i(106720),	-- Stonecrag Breastplate
										i(106708),	-- Voidwrap Robe
										-- Wrist
										i(106729),	-- Meadowstomper Bindings
										i(106713),	-- Sunspring Bracers							
										i(106711),	-- Voidwrap Wristwraps
										-- Hands
										i(106747),	-- Packrunner Gauntlets
										i(106699),	-- Sabermaw Gloves
										i(106714),	-- Sunspring Gauntlets
										i(106706),	-- Voidwrap Handwraps
										-- Waist
										i(106722),	-- Stonecrag Girdle
										i(106712),	-- Sunspring Belt
										i(106704),	-- Voidwrap Cord
										-- Legs
										i(106740),	-- Dark Star Leggings
										i(106691),	-- Ironfist Legplates
										i(106717),	-- Sunspring Legguards
										-- Feet
										i(106697),	-- Sabermaw Boots
										i(106726),	-- Stonecrag Sabatons
									}),
								},
							},
							{	-- Adventurer's Pack
								["objectID"] = 233511,	-- Adventurer's Pack
								["questID"] = 35969,
								["coords"] = {
									{ 45.6, 52.0, 550 },
								},
								["g"] = {
									-- Weapons
									i(116627),	-- Howling Axe
									i(116629),	-- Howling Bow
									i(116628),	-- Howling Broadaxe
									i(116630),	-- Howling Crossbow
									i(116631),	-- Howling Dagger
									i(116642),	-- Howling Greatsword
									i(116634),	-- Howling Gun
									i(116637),	-- Howling Hammer
									i(116633),	-- Howling Knuckles
									i(116635),	-- Howling Mace
									i(116638),	-- Howling Spear
									i(116640),	-- Howling Staff
									i(116639),	-- Howling Staff
									i(116641),	-- Howling Sword
									i(116724),	-- Oshu'gun Basin
									i(116636),	-- Oshu'gun Scepter
									i(116632),	-- Oshu'gun Spellblade
									i(116643),	-- Oshu'gun Wand
									i(106727),	-- Stonecrag Shield
									-- Head
									i(106738),	-- Dark Star Cowl
									i(106733),	-- Meadowstomper Hood
									i(106749),	-- Packrunner Helm
									i(106723),	-- Stonecrag Helm
									-- Neck
									i(116687),	-- Oshu'gun Amulet
									-- Shoulders
									i(106743),	-- Dark Star Shoulderpads
									i(106735),	-- Meadowstomper Shoulders
									i(106751),	-- Packrunner Spaulders
									i(106725),	-- Stonecrag Pauldrons
									-- Back
									i(116723),	-- Oshu'gun Cloak
									-- Chest
									i(106741),	-- Dark Star Robe
									i(106734),	-- Meadowstomper Jerkin
									i(106752),	-- Packrunner Vest
									i(106720),	-- Stonecrag Breastplate
									-- Wrist
									i(106744),	-- Dark Star Wristwraps
									i(106729),	-- Meadowstomper Bindings
									i(106746),	-- Packrunner Bracers
									i(106728),	-- Stonecrag Vambraces
									-- Hands
									i(106739),	-- Dark Star Handwraps
									i(106732),	-- Meadowstomper Gloves
									i(106747),	-- Packrunner Gauntlets
									i(106721),	-- Stonecrag Gauntlets
									-- Waist
									i(106737),	-- Dark Star Cord
									i(106736),	-- Meadowstomper Waistband
									i(106745),	-- Packrunner Belt
									i(106722),	-- Stonecrag Girdle
									-- Legs
									i(106740),	-- Dark Star Leggings							
									i(106731),	-- Meadowstomper Britches
									i(106750),	-- Packrunner Legguards
									i(106724),	-- Stonecrag Legplates
									-- Feet
									i(106742),	-- Dark Star Sandals
									i(106730),	-- Meadowstomper Boots
									i(106748),	-- Packrunner Greaves
									i(106726),	-- Stonecrag Sabatons
									-- Finger
									i(116725),	-- Oshu'gun Ring
									i(116920, {	-- True Steel Lockbox
										-- Weapons
										i(116721),	-- Ancestral Branch
										i(116619),	-- Ancestral Scepter
										i(116626),	-- Ancestral Wand
										i(116613),	-- Gorian Crossbow
										i(116614),	-- Gorian Dagger
										i(116625),	-- Gorian Greatsword
										i(116617),	-- Gorian Gun
										i(116620),	-- Gorian Hammer
										i(116618),	-- Gorian Mace
										i(116621),	-- Gorian Spear
										i(116622),	-- Gorian Staff
										i(116627),	-- Howling Axe
										i(116634),	-- Howling Gun
										i(116633),	-- Howling Knuckles
										i(116640),	-- Howling Staff
										i(116632),	-- Oshu'gun Spellblade
										i(116643),	-- Oshu'gun Wand
										-- Head
										i(106690),	-- Ironfist Helm
										i(106723),	-- Stonecrag Helm
										-- Shoulders
										i(106692),	-- Ironfist Pauldrons
										i(106735),	-- Meadowstomper Shoulders
										i(106702),	-- Sabermaw Shoulders
										-- Chest
										i(106687),	-- Ironfist Breastplate
										i(106701),	-- Sabermaw Jerkin
										i(106720),	-- Stonecrag Breastplate
										i(106708),	-- Voidwrap Robe
										-- Wrist
										i(106729),	-- Meadowstomper Bindings
										i(106713),	-- Sunspring Bracers							
										i(106711),	-- Voidwrap Wristwraps
										-- Hands
										i(106747),	-- Packrunner Gauntlets
										i(106699),	-- Sabermaw Gloves
										i(106714),	-- Sunspring Gauntlets
										i(106706),	-- Voidwrap Handwraps
										-- Waist
										i(106722),	-- Stonecrag Girdle
										i(106712),	-- Sunspring Belt
										i(106704),	-- Voidwrap Cord
										-- Legs
										i(106740),	-- Dark Star Leggings
										i(106691),	-- Ironfist Legplates
										i(106717),	-- Sunspring Legguards
										-- Feet
										i(106697),	-- Sabermaw Boots
										i(106726),	-- Stonecrag Sabatons
									}),							
								},
							},
							{	-- Adventurer's Pack
								["objectID"] = 232406,	-- Adventurer's Pack
								["questID"] = 35597,
								["coords"] = {
									{ 69.9, 52.5, 550 },
								},
								["g"] = {
									-- Weapons
									i(116627),	-- Howling Axe
									i(116629),	-- Howling Bow
									i(116628),	-- Howling Broadaxe
									i(116630),	-- Howling Crossbow
									i(116631),	-- Howling Dagger
									i(116642),	-- Howling Greatsword
									i(116634),	-- Howling Gun
									i(116637),	-- Howling Hammer
									i(116633),	-- Howling Knuckles
									i(116635),	-- Howling Mace
									i(116638),	-- Howling Spear
									i(116640),	-- Howling Staff
									i(116639),	-- Howling Staff
									i(116641),	-- Howling Sword
									i(116724),	-- Oshu'gun Basin
									i(116636),	-- Oshu'gun Scepter
									i(116632),	-- Oshu'gun Spellblade
									i(116643),	-- Oshu'gun Wand
									i(106727),	-- Stonecrag Shield
									-- Head
									i(106738),	-- Dark Star Cowl
									i(106733),	-- Meadowstomper Hood
									i(106749),	-- Packrunner Helm
									i(106723),	-- Stonecrag Helm
									-- Neck
									i(116687),	-- Oshu'gun Amulet
									-- Shoulders
									i(106743),	-- Dark Star Shoulderpads
									i(106735),	-- Meadowstomper Shoulders
									i(106751),	-- Packrunner Spaulders
									i(106725),	-- Stonecrag Pauldrons
									-- Back
									i(116723),	-- Oshu'gun Cloak
									-- Chest
									i(106741),	-- Dark Star Robe
									i(106734),	-- Meadowstomper Jerkin
									i(106752),	-- Packrunner Vest
									i(106720),	-- Stonecrag Breastplate
									-- Wrist
									i(106744),	-- Dark Star Wristwraps
									i(106729),	-- Meadowstomper Bindings
									i(106746),	-- Packrunner Bracers
									i(106728),	-- Stonecrag Vambraces
									-- Hands
									i(106739),	-- Dark Star Handwraps
									i(106732),	-- Meadowstomper Gloves
									i(106747),	-- Packrunner Gauntlets
									i(106721),	-- Stonecrag Gauntlets
									-- Waist
									i(106737),	-- Dark Star Cord
									i(106736),	-- Meadowstomper Waistband
									i(106745),	-- Packrunner Belt
									i(106722),	-- Stonecrag Girdle
									-- Legs
									i(106740),	-- Dark Star Leggings							
									i(106731),	-- Meadowstomper Britches
									i(106750),	-- Packrunner Legguards
									i(106724),	-- Stonecrag Legplates
									-- Feet
									i(106742),	-- Dark Star Sandals
									i(106730),	-- Meadowstomper Boots
									i(106748),	-- Packrunner Greaves
									i(106726),	-- Stonecrag Sabatons
									-- Finger
									i(116725),	-- Oshu'gun Ring
									i(116920, {	-- True Steel Lockbox
										-- Weapons
										i(116721),	-- Ancestral Branch
										i(116619),	-- Ancestral Scepter
										i(116626),	-- Ancestral Wand
										i(116613),	-- Gorian Crossbow
										i(116614),	-- Gorian Dagger
										i(116625),	-- Gorian Greatsword
										i(116617),	-- Gorian Gun
										i(116620),	-- Gorian Hammer
										i(116618),	-- Gorian Mace
										i(116621),	-- Gorian Spear
										i(116622),	-- Gorian Staff
										i(116627),	-- Howling Axe
										i(116634),	-- Howling Gun
										i(116633),	-- Howling Knuckles
										i(116640),	-- Howling Staff
										i(116632),	-- Oshu'gun Spellblade
										i(116643),	-- Oshu'gun Wand
										-- Head
										i(106690),	-- Ironfist Helm
										i(106723),	-- Stonecrag Helm
										-- Shoulders
										i(106692),	-- Ironfist Pauldrons
										i(106735),	-- Meadowstomper Shoulders
										i(106702),	-- Sabermaw Shoulders
										-- Chest
										i(106687),	-- Ironfist Breastplate
										i(106701),	-- Sabermaw Jerkin
										i(106720),	-- Stonecrag Breastplate
										i(106708),	-- Voidwrap Robe
										-- Wrist
										i(106729),	-- Meadowstomper Bindings
										i(106713),	-- Sunspring Bracers							
										i(106711),	-- Voidwrap Wristwraps
										-- Hands
										i(106747),	-- Packrunner Gauntlets
										i(106699),	-- Sabermaw Gloves
										i(106714),	-- Sunspring Gauntlets
										i(106706),	-- Voidwrap Handwraps
										-- Waist
										i(106722),	-- Stonecrag Girdle
										i(106712),	-- Sunspring Belt
										i(106704),	-- Voidwrap Cord
										-- Legs
										i(106740),	-- Dark Star Leggings
										i(106691),	-- Ironfist Legplates
										i(106717),	-- Sunspring Legguards
										-- Feet
										i(106697),	-- Sabermaw Boots
										i(106726),	-- Stonecrag Sabatons
									}),
								},
							},
							{	-- Adventurer's Pouch
								["objectID"] = 233658,	-- Adventurer's Pouch
								["questID"] = 36088,
								["modelID"] = 32854,
								["description"]	= "Follow coords in order, start at cave and follow ramp to reach the pouch.",
								["coords"] = {
									{ 56.47, 61.80, 550 },	-- Cave Entrance
									{ 64.26, 37.92, 550 },	-- Start of Ramp
									{ 65.89, 57.66, 550 },	-- Pouch
								},
								["g"] = {
									-- Weapons
									i(116627),	-- Howling Axe
									i(116629),	-- Howling Bow
									i(116628),	-- Howling Broadaxe
									i(116630),	-- Howling Crossbow
									i(116631),	-- Howling Dagger
									i(116642),	-- Howling Greatsword
									i(116634),	-- Howling Gun
									i(116637),	-- Howling Hammer
									i(116633),	-- Howling Knuckles
									i(116635),	-- Howling Mace
									i(116638),	-- Howling Spear
									i(116640),	-- Howling Staff
									i(116639),	-- Howling Staff
									i(116641),	-- Howling Sword
									i(116724),	-- Oshu'gun Basin
									i(116636),	-- Oshu'gun Scepter
									i(116632),	-- Oshu'gun Spellblade
									i(116643),	-- Oshu'gun Wand
									i(106727),	-- Stonecrag Shield
									-- Head
									i(106738),	-- Dark Star Cowl
									i(106733),	-- Meadowstomper Hood
									i(106749),	-- Packrunner Helm
									i(106723),	-- Stonecrag Helm
									-- Neck
									i(116687),	-- Oshu'gun Amulet
									-- Shoulders
									i(106743),	-- Dark Star Shoulderpads
									i(106735),	-- Meadowstomper Shoulders
									i(106751),	-- Packrunner Spaulders
									i(106725),	-- Stonecrag Pauldrons
									-- Back
									i(116723),	-- Oshu'gun Cloak
									-- Chest
									i(106741),	-- Dark Star Robe
									i(106734),	-- Meadowstomper Jerkin
									i(106752),	-- Packrunner Vest
									i(106720),	-- Stonecrag Breastplate
									-- Wrist
									i(106744),	-- Dark Star Wristwraps
									i(106729),	-- Meadowstomper Bindings
									i(106746),	-- Packrunner Bracers
									i(106728),	-- Stonecrag Vambraces
									-- Hands
									i(106739),	-- Dark Star Handwraps
									i(106732),	-- Meadowstomper Gloves
									i(106747),	-- Packrunner Gauntlets
									i(106721),	-- Stonecrag Gauntlets
									-- Waist
									i(106737),	-- Dark Star Cord
									i(106736),	-- Meadowstomper Waistband
									i(106745),	-- Packrunner Belt
									i(106722),	-- Stonecrag Girdle
									-- Legs
									i(106740),	-- Dark Star Leggings							
									i(106731),	-- Meadowstomper Britches
									i(106750),	-- Packrunner Legguards
									i(106724),	-- Stonecrag Legplates
									-- Feet
									i(106742),	-- Dark Star Sandals
									i(106730),	-- Meadowstomper Boots
									i(106748),	-- Packrunner Greaves
									i(106726),	-- Stonecrag Sabatons
									-- Finger
									i(116725),	-- Oshu'gun Ring
									i(116920, {	-- True Steel Lockbox
										-- Weapons
										i(116721),	-- Ancestral Branch
										i(116619),	-- Ancestral Scepter
										i(116626),	-- Ancestral Wand
										i(116613),	-- Gorian Crossbow
										i(116614),	-- Gorian Dagger
										i(116625),	-- Gorian Greatsword
										i(116617),	-- Gorian Gun
										i(116620),	-- Gorian Hammer
										i(116618),	-- Gorian Mace
										i(116621),	-- Gorian Spear
										i(116622),	-- Gorian Staff
										i(116627),	-- Howling Axe
										i(116634),	-- Howling Gun
										i(116633),	-- Howling Knuckles
										i(116640),	-- Howling Staff
										i(116632),	-- Oshu'gun Spellblade
										i(116643),	-- Oshu'gun Wand
										-- Head
										i(106690),	-- Ironfist Helm
										i(106723),	-- Stonecrag Helm
										-- Shoulders
										i(106692),	-- Ironfist Pauldrons
										i(106735),	-- Meadowstomper Shoulders
										i(106702),	-- Sabermaw Shoulders
										-- Chest
										i(106687),	-- Ironfist Breastplate
										i(106701),	-- Sabermaw Jerkin
										i(106720),	-- Stonecrag Breastplate
										i(106708),	-- Voidwrap Robe
										-- Wrist
										i(106729),	-- Meadowstomper Bindings
										i(106713),	-- Sunspring Bracers							
										i(106711),	-- Voidwrap Wristwraps
										-- Hands
										i(106747),	-- Packrunner Gauntlets
										i(106699),	-- Sabermaw Gloves
										i(106714),	-- Sunspring Gauntlets
										i(106706),	-- Voidwrap Handwraps
										-- Waist
										i(106722),	-- Stonecrag Girdle
										i(106712),	-- Sunspring Belt
										i(106704),	-- Voidwrap Cord
										-- Legs
										i(106740),	-- Dark Star Leggings
										i(106691),	-- Ironfist Legplates
										i(106717),	-- Sunspring Legguards
										-- Feet
										i(106697),	-- Sabermaw Boots
										i(106726),	-- Stonecrag Sabatons
									}),						
								},
							},
							{	-- Adventurer's Pouch
								["objectID"] = 233623,	-- Adventurer's Pouch
								["questID"] = 36050,
								["coords"] = {
									{ 56.6, 72.9, 550 },
								},
								["modelID"] = 32854,
								["g"] = {
									-- Weapons
									i(116627),	-- Howling Axe
									i(116629),	-- Howling Bow
									i(116628),	-- Howling Broadaxe
									i(116630),	-- Howling Crossbow
									i(116631),	-- Howling Dagger
									i(116642),	-- Howling Greatsword
									i(116634),	-- Howling Gun
									i(116637),	-- Howling Hammer
									i(116633),	-- Howling Knuckles
									i(116635),	-- Howling Mace
									i(116638),	-- Howling Spear
									i(116640),	-- Howling Staff
									i(116639),	-- Howling Staff
									i(116641),	-- Howling Sword
									i(116724),	-- Oshu'gun Basin
									i(116636),	-- Oshu'gun Scepter
									i(116632),	-- Oshu'gun Spellblade
									i(116643),	-- Oshu'gun Wand
									i(106727),	-- Stonecrag Shield
									-- Head
									i(106738),	-- Dark Star Cowl
									i(106733),	-- Meadowstomper Hood
									i(106749),	-- Packrunner Helm
									i(106723),	-- Stonecrag Helm
									-- Neck
									i(116687),	-- Oshu'gun Amulet
									-- Shoulders
									i(106743),	-- Dark Star Shoulderpads
									i(106735),	-- Meadowstomper Shoulders
									i(106751),	-- Packrunner Spaulders
									i(106725),	-- Stonecrag Pauldrons
									-- Back
									i(116723),	-- Oshu'gun Cloak
									-- Chest
									i(106741),	-- Dark Star Robe
									i(106734),	-- Meadowstomper Jerkin
									i(106752),	-- Packrunner Vest
									i(106720),	-- Stonecrag Breastplate
									-- Wrist
									i(106744),	-- Dark Star Wristwraps
									i(106729),	-- Meadowstomper Bindings
									i(106746),	-- Packrunner Bracers
									i(106728),	-- Stonecrag Vambraces
									-- Hands
									i(106739),	-- Dark Star Handwraps
									i(106732),	-- Meadowstomper Gloves
									i(106747),	-- Packrunner Gauntlets
									i(106721),	-- Stonecrag Gauntlets
									-- Waist
									i(106737),	-- Dark Star Cord
									i(106736),	-- Meadowstomper Waistband
									i(106745),	-- Packrunner Belt
									i(106722),	-- Stonecrag Girdle
									-- Legs
									i(106740),	-- Dark Star Leggings							
									i(106731),	-- Meadowstomper Britches
									i(106750),	-- Packrunner Legguards
									i(106724),	-- Stonecrag Legplates
									-- Feet
									i(106742),	-- Dark Star Sandals
									i(106730),	-- Meadowstomper Boots
									i(106748),	-- Packrunner Greaves
									i(106726),	-- Stonecrag Sabatons
									-- Finger
									i(116725),	-- Oshu'gun Ring
									i(116920, {	-- True Steel Lockbox
										-- Weapons
										i(116721),	-- Ancestral Branch
										i(116619),	-- Ancestral Scepter
										i(116626),	-- Ancestral Wand
										i(116613),	-- Gorian Crossbow
										i(116614),	-- Gorian Dagger
										i(116625),	-- Gorian Greatsword
										i(116617),	-- Gorian Gun
										i(116620),	-- Gorian Hammer
										i(116618),	-- Gorian Mace
										i(116621),	-- Gorian Spear
										i(116622),	-- Gorian Staff
										i(116627),	-- Howling Axe
										i(116634),	-- Howling Gun
										i(116633),	-- Howling Knuckles
										i(116640),	-- Howling Staff
										i(116632),	-- Oshu'gun Spellblade
										i(116643),	-- Oshu'gun Wand
										-- Head
										i(106690),	-- Ironfist Helm
										i(106723),	-- Stonecrag Helm
										-- Shoulders
										i(106692),	-- Ironfist Pauldrons
										i(106735),	-- Meadowstomper Shoulders
										i(106702),	-- Sabermaw Shoulders
										-- Chest
										i(106687),	-- Ironfist Breastplate
										i(106701),	-- Sabermaw Jerkin
										i(106720),	-- Stonecrag Breastplate
										i(106708),	-- Voidwrap Robe
										-- Wrist
										i(106729),	-- Meadowstomper Bindings
										i(106713),	-- Sunspring Bracers							
										i(106711),	-- Voidwrap Wristwraps
										-- Hands
										i(106747),	-- Packrunner Gauntlets
										i(106699),	-- Sabermaw Gloves
										i(106714),	-- Sunspring Gauntlets
										i(106706),	-- Voidwrap Handwraps
										-- Waist
										i(106722),	-- Stonecrag Girdle
										i(106712),	-- Sunspring Belt
										i(106704),	-- Voidwrap Cord
										-- Legs
										i(106740),	-- Dark Star Leggings
										i(106691),	-- Ironfist Legplates
										i(106717),	-- Sunspring Legguards
										-- Feet
										i(106697),	-- Sabermaw Boots
										i(106726),	-- Stonecrag Sabatons
									}),								
								},
							},
							{	-- Adventurer's Sack
								["objectID"] = 233499,	-- Adventurer's Sack
								["questID"] = 35955,
								["coords"] = {
									{ 73.9, 14.1, 550 },
								},
								["modelID"] = 32854,
								["g"] = {
									-- Weapons
									i(116627),	-- Howling Axe
									i(116629),	-- Howling Bow
									i(116628),	-- Howling Broadaxe
									i(116630),	-- Howling Crossbow
									i(116631),	-- Howling Dagger
									i(116642),	-- Howling Greatsword
									i(116634),	-- Howling Gun
									i(116637),	-- Howling Hammer
									i(116633),	-- Howling Knuckles
									i(116635),	-- Howling Mace
									i(116638),	-- Howling Spear
									i(116640),	-- Howling Staff
									i(116639),	-- Howling Staff
									i(116641),	-- Howling Sword
									i(116724),	-- Oshu'gun Basin
									i(116636),	-- Oshu'gun Scepter
									i(116632),	-- Oshu'gun Spellblade
									i(116643),	-- Oshu'gun Wand
									i(106727),	-- Stonecrag Shield
									-- Head
									i(106738),	-- Dark Star Cowl
									i(106733),	-- Meadowstomper Hood
									i(106749),	-- Packrunner Helm
									i(106723),	-- Stonecrag Helm
									-- Neck
									i(116687),	-- Oshu'gun Amulet
									-- Shoulders
									i(106743),	-- Dark Star Shoulderpads
									i(106735),	-- Meadowstomper Shoulders
									i(106751),	-- Packrunner Spaulders
									i(106725),	-- Stonecrag Pauldrons
									-- Back
									i(116723),	-- Oshu'gun Cloak
									-- Chest
									i(106741),	-- Dark Star Robe
									i(106734),	-- Meadowstomper Jerkin
									i(106752),	-- Packrunner Vest
									i(106720),	-- Stonecrag Breastplate
									-- Wrist
									i(106744),	-- Dark Star Wristwraps
									i(106729),	-- Meadowstomper Bindings
									i(106746),	-- Packrunner Bracers
									i(106728),	-- Stonecrag Vambraces
									-- Hands
									i(106739),	-- Dark Star Handwraps
									i(106732),	-- Meadowstomper Gloves
									i(106747),	-- Packrunner Gauntlets
									i(106721),	-- Stonecrag Gauntlets
									-- Waist
									i(106737),	-- Dark Star Cord
									i(106736),	-- Meadowstomper Waistband
									i(106745),	-- Packrunner Belt
									i(106722),	-- Stonecrag Girdle
									-- Legs
									i(106740),	-- Dark Star Leggings							
									i(106731),	-- Meadowstomper Britches
									i(106750),	-- Packrunner Legguards
									i(106724),	-- Stonecrag Legplates
									-- Feet
									i(106742),	-- Dark Star Sandals
									i(106730),	-- Meadowstomper Boots
									i(106748),	-- Packrunner Greaves
									i(106726),	-- Stonecrag Sabatons
									-- Finger
									i(116725),	-- Oshu'gun Ring
									i(116920, {	-- True Steel Lockbox
										-- Weapons
										i(116721),	-- Ancestral Branch
										i(116619),	-- Ancestral Scepter
										i(116626),	-- Ancestral Wand
										i(116613),	-- Gorian Crossbow
										i(116614),	-- Gorian Dagger
										i(116625),	-- Gorian Greatsword
										i(116617),	-- Gorian Gun
										i(116620),	-- Gorian Hammer
										i(116618),	-- Gorian Mace
										i(116621),	-- Gorian Spear
										i(116622),	-- Gorian Staff
										i(116627),	-- Howling Axe
										i(116634),	-- Howling Gun
										i(116633),	-- Howling Knuckles
										i(116640),	-- Howling Staff
										i(116632),	-- Oshu'gun Spellblade
										i(116643),	-- Oshu'gun Wand
										-- Head
										i(106690),	-- Ironfist Helm
										i(106723),	-- Stonecrag Helm
										-- Shoulders
										i(106692),	-- Ironfist Pauldrons
										i(106735),	-- Meadowstomper Shoulders
										i(106702),	-- Sabermaw Shoulders
										-- Chest
										i(106687),	-- Ironfist Breastplate
										i(106701),	-- Sabermaw Jerkin
										i(106720),	-- Stonecrag Breastplate
										i(106708),	-- Voidwrap Robe
										-- Wrist
										i(106729),	-- Meadowstomper Bindings
										i(106713),	-- Sunspring Bracers							
										i(106711),	-- Voidwrap Wristwraps
										-- Hands
										i(106747),	-- Packrunner Gauntlets
										i(106699),	-- Sabermaw Gloves
										i(106714),	-- Sunspring Gauntlets
										i(106706),	-- Voidwrap Handwraps
										-- Waist
										i(106722),	-- Stonecrag Girdle
										i(106712),	-- Sunspring Belt
										i(106704),	-- Voidwrap Cord
										-- Legs
										i(106740),	-- Dark Star Leggings
										i(106691),	-- Ironfist Legplates
										i(106717),	-- Sunspring Legguards
										-- Feet
										i(106697),	-- Sabermaw Boots
										i(106726),	-- Stonecrag Sabatons
									}),								
								},
							},
							{	-- Adventurer's Staff
								["objectID"] = 233457,	-- Adventurer's Staff
								["questID"] = 35953,
								["coords"] = {
									{ 81.5, 13.0, 550 },
								},
								["g"] = {
									i(116640),  -- Howling Staff
								},
							},
							{	-- Ancient Blademaster
								["npcID"] = 82899,	-- Ancient Blademaster
								["questID"] = 35778,
								["coords"] = {
									{ 84.6, 53.6, 550 },
								},
								["g"] = {
									i(116832),	-- Blademaster's Honor
								},
							},
							{	-- Berserk T-300 Series Mark II
								["npcID"] = 82826,	-- Berserk T-300 Series Mark II
								["questID"] = 35735,
								["coords"] = {
									{ 76.96, 64.37, 550 },
								},
								["g"] = {
									i(116823),	-- Katealystic Konverter
								},
							},
							{	-- Bonebreaker
								["npcID"] = 87837,	-- Bonebreaker
								["questID"] = 37396,
								["isDaily"] = true,							
								["coords"] = {
									{ 39.2, 15.8, 550 },
									{ 39.8, 13.2, 550 },
								},
								["g"] = {
									{	-- Bonebreaker
										["achievementID"] = 9571,	-- Broke Back Precipice (Broken Precipice)
										["criteriaID"] = 2,			-- Bonebreaker
									},
									{	-- Rattlekilt
										["itemID"] = 119370,	-- Rattlekilt
									},
								},
							}, 
							{	-- Bone-Carved Dagger
								["objectID"] = 233532,	-- Bone-Carved Dagger
								["questID"] = 35986,
								["coords"] = {
									{ 77.3, 28.1, 550 },
								},
								["g"] = {
									i(116760),	-- Saberon-Fang Shanker
								},
							},						
							{	-- Brutag Grimblade	**Coords uncofirmed, relied on wowhead**
								["npcID"] = 87234,	-- Brutag Grimblade
								["questID"] = 37400,
								["isDaily"] = true,							
								["coords"] = {
									{ 43.0, 36.2, 550 },
								},
								["g"] = {
									{	-- Brutag Grimblade
										["achievementID"] = 9541,	-- The Song of Silence (Mok'gol Outpost)
										["criteriaID"] = 2,			-- Brutag Grimblade
									},
									{	-- Brutag's Iron Toe Boots
										["itemID"] = 119380,	-- Brutag's Iron Toe Boots
									},
								},
							}, 
							{	-- Burning Blade Cache
								["objectID"] = 233137,	-- Burning Blade Cache
								["questID"] = 35696,
								["coords"] = {
									{ 85.4, 53.46, 550 },
								},
								["g"] = {
									-- Weapons
									i(116627),	-- Howling Axe
									i(116629),	-- Howling Bow
									i(116628),	-- Howling Broadaxe
									i(116630),	-- Howling Crossbow
									i(116631),	-- Howling Dagger
									i(116642),	-- Howling Greatsword
									i(116634),	-- Howling Gun
									i(116637),	-- Howling Hammer
									i(116633),	-- Howling Knuckles
									i(116635),	-- Howling Mace
									i(116638),	-- Howling Spear
									i(116640),	-- Howling Staff
									i(116639),	-- Howling Staff
									i(116641),	-- Howling Sword
									i(116724),	-- Oshu'gun Basin
									i(116636),	-- Oshu'gun Scepter
									i(116632),	-- Oshu'gun Spellblade
									i(116643),	-- Oshu'gun Wand
									i(106727),	-- Stonecrag Shield
									-- Head
									i(106738),	-- Dark Star Cowl
									i(106733),	-- Meadowstomper Hood
									i(106749),	-- Packrunner Helm
									i(106723),	-- Stonecrag Helm
									-- Neck
									i(116687),	-- Oshu'gun Amulet
									-- Shoulders
									i(106743),	-- Dark Star Shoulderpads
									i(106735),	-- Meadowstomper Shoulders
									i(106751),	-- Packrunner Spaulders
									i(106725),	-- Stonecrag Pauldrons
									-- Back
									i(116723),	-- Oshu'gun Cloak
									-- Chest
									i(106741),	-- Dark Star Robe
									i(106734),	-- Meadowstomper Jerkin
									i(106752),	-- Packrunner Vest
									i(106720),	-- Stonecrag Breastplate
									-- Wrist
									i(106744),	-- Dark Star Wristwraps
									i(106729),	-- Meadowstomper Bindings
									i(106746),	-- Packrunner Bracers
									i(106728),	-- Stonecrag Vambraces
									-- Hands
									i(106739),	-- Dark Star Handwraps
									i(106732),	-- Meadowstomper Gloves
									i(106747),	-- Packrunner Gauntlets
									i(106721),	-- Stonecrag Gauntlets
									-- Waist
									i(106737),	-- Dark Star Cord
									i(106736),	-- Meadowstomper Waistband
									i(106745),	-- Packrunner Belt
									i(106722),	-- Stonecrag Girdle
									-- Legs
									i(106740),	-- Dark Star Leggings							
									i(106731),	-- Meadowstomper Britches
									i(106750),	-- Packrunner Legguards
									i(106724),	-- Stonecrag Legplates
									-- Feet
									i(106742),	-- Dark Star Sandals
									i(106730),	-- Meadowstomper Boots
									i(106748),	-- Packrunner Greaves
									i(106726),	-- Stonecrag Sabatons
									-- Finger
									i(116725),	-- Oshu'gun Ring
									i(116920, {	-- True Steel Lockbox
										-- Weapons
										i(116721),	-- Ancestral Branch
										i(116619),	-- Ancestral Scepter
										i(116626),	-- Ancestral Wand
										i(116613),	-- Gorian Crossbow
										i(116614),	-- Gorian Dagger
										i(116625),	-- Gorian Greatsword
										i(116617),	-- Gorian Gun
										i(116620),	-- Gorian Hammer
										i(116618),	-- Gorian Mace
										i(116621),	-- Gorian Spear
										i(116622),	-- Gorian Staff
										i(116627),	-- Howling Axe
										i(116634),	-- Howling Gun
										i(116633),	-- Howling Knuckles
										i(116640),	-- Howling Staff
										i(116632),	-- Oshu'gun Spellblade
										i(116643),	-- Oshu'gun Wand
										-- Head
										i(106690),	-- Ironfist Helm
										i(106723),	-- Stonecrag Helm
										-- Shoulders
										i(106692),	-- Ironfist Pauldrons
										i(106735),	-- Meadowstomper Shoulders
										i(106702),	-- Sabermaw Shoulders
										-- Chest
										i(106687),	-- Ironfist Breastplate
										i(106701),	-- Sabermaw Jerkin
										i(106720),	-- Stonecrag Breastplate
										i(106708),	-- Voidwrap Robe
										-- Wrist
										i(106729),	-- Meadowstomper Bindings
										i(106713),	-- Sunspring Bracers							
										i(106711),	-- Voidwrap Wristwraps
										-- Hands
										i(106747),	-- Packrunner Gauntlets
										i(106699),	-- Sabermaw Gloves
										i(106714),	-- Sunspring Gauntlets
										i(106706),	-- Voidwrap Handwraps
										-- Waist
										i(106722),	-- Stonecrag Girdle
										i(106712),	-- Sunspring Belt
										i(106704),	-- Voidwrap Cord
										-- Legs
										i(106740),	-- Dark Star Leggings
										i(106691),	-- Ironfist Legplates
										i(106717),	-- Sunspring Legguards
										-- Feet
										i(106697),	-- Sabermaw Boots
										i(106726),	-- Stonecrag Sabatons
									}),	
								},
							},
							{	-- Captain Ironbeard
								["npcID"] = 79725,	-- Captain Ironbeard
								["questID"] = 34727,
								["coords"] = {
									{ 34.6, 77.0, 550 },
								},
								["isDaily"] = true,						
								["g"] = {	
									i(118244),	-- Iron Buccaneer's Hat Toy
									i(116809),	-- Ironbeard's Blunderbuss
								},
							}, 
							{	-- Durg Spinecrusher
								["npcID"] = 87788,	-- Durg Spinecrusher
								["questID"] = 37395,
								["isDaily"] = true,							
								["coords"] = {	-- patrols in a circle
									{ 38.8, 23.0, 550 },
									{ 35.6, 20.6, 550 },
								},
								["g"] = {
									{	-- Durg Spinecrusher
										["achievementID"] = 9571,	-- Broke Back Precipice (Broken Precipice)
										["criteriaID"] = 1,			-- Durg Spinecrusher
									},
									{	-- Durg's Heavy Maul
										["itemID"] = 119405,	-- Durg's Heavy Maul
									},
								},
							}, 
							{	-- Edge of Reality	**Coords uncofirmed, relied on wowhead**
								["objectID"] = 239828,	-- Edge of Reality
								["g"] = {
									{	-- Voidtalon Egg
										["objectID"] = 239901,	-- Voidtalon Egg
										["modelScale"] = 2,
										["model"] = "World\\Expansion06\\Doodads\\Dungeon\\Doodads\\7du_Nightmare_Egg01.mdx",
										["g"] = {	
											i(121815),	-- Voidtalon of the Dark Star Mount
										},
									},
								},
								["coords"] = {
									{ 40.5, 47.6, 550 },
									{ 44.0, 30.7, 550 },
									{ 45.9, 31.4, 550 },
									{ 57.3, 26.7, 550 },
									{ 59.5, 10.2, 550 },
								},
								["description"] = "Click on Edge of Reality, get teleported, click on egg for mount.|nIf you are on a realm not of your own when you click on Edge of Reality the mount will be mailed to you as you will not be teleported. |r",
							},										
							{	-- Elemental Shackles
								["objectID"] = 233598,	-- Elemental Shackles
								["questID"] = 36036,
								["coords"] = {
									{ 78.9, 15.5, 550 },
								},
								["g"] = {
									i(118251),	-- Trophy Gemstone of the Elements
								},
							},
							{	-- Flinthide
								["npcID"] = 83483,	-- Flinthide
								["questID"] = 35893,
								["coords"] = {
									{ 69.6, 42.0, 550 }, 						
								},
								["g"] = {	
									i(116807),	-- Flinthide's Impenetrable Crest
								},
							},
							{	-- Fragment of Oshu'gun
								["objectID"] = 233560,	-- Fragment of Oshu'gun
								["questID"] = 36020,
								["coords"] = {
									{ 45.8, 66.2, 550 },
								},
								["g"] = {
									i(117981),	-- Fragment of Oshu'gun
								},
							},						
							{	-- Gar'lua <The Wolfmother>
								["npcID"] = 82764,	-- Gar'lua <The Wolfmother>
								["questID"] = 35715,
								["coords"] = {
									{ 52.45, 55.77, 550 },
								},
								["g"] = {
									i(118246),	-- Call of the Wolfmother
								},
							},
							{	-- Gaz'orda
								["npcID"] = 80122,	-- Gaz'orda
								["coords"] = {
									{ 43.8, 77.7, 550 },
								},
								["g"] = {
									i(116798),	-- Gaz'orda's Grim Gaze
								},
							},
							{	-- Gnarlhoof the Rabid
								["npcID"] = 82778,	-- Gnarlhoof the Rabid
								["questID"] = 35717,
								["coords"] = {
									{ 66.68, 56.31, 550 },
								},
								["g"] = {
									i(116824),	-- Rabid Talbuk Horn
								},
							},
							{	-- Gorepetal
								["npcID"] = 83509,	-- Gorepetal
								["questID"] = 35898,
								["coords"] = {
									{ 93.2, 28.2, 550 },
								},
								["g"] = {	
									i(116916),	-- Gorepetal's Gentle Grasp
								},
							},				
							{	-- Gortag Steelgrip
								["npcID"] = 87344,	-- Gortag Steelgrip
								["questID"] = 37472,
								["isDaily"] = true,
								["coords"] = {
									{ 42.6, 36.2, 550 },
								},
								["description"] = "Obtain random area drop Secret Meeting Details and then blow the Signal Horn at coords 42.1, 36.84 to summon Gortag and Krahl.",
								["g"] = {
									{	-- Gortag Steelgrip
										["achievementID"] = 9541,	-- The Song of Silence (Mok'gol Outpost)
										["criteriaID"] = 4,			-- Gortag Steelgrip
									},
								},
							},
							{	-- Graveltooth
								["npcID"] = 84263,	-- Graveltooth
								["questID"] = 36159,	
								["coords"] = {
									{ 84.0, 36.8, 550 },
								},
								["description"] = "Kill about 15 goren in the area to draw out Graveltooth, he will yell 3 times before appearing.",
								["g"] = {	
									i(118689),	-- Graveltooth's Manacles
								},
							}, 
							{	-- Greatfeather
								["npcID"] = 82758,	-- Greatfeather
								["questID"] = 35714,
								["coords"] = {
									{ 66.8, 51.2, 550 },
								},
								["g"] = {	
									i(116795),	-- Greatfeather's Down Robe
								},
							}, 
							{	-- Grizzlemaw
								["npcID"] = 82912,	-- Grizzlemaw
								["questID"] = 35784,
								["coords"] = {
									{ 89.4, 72.6, 550 },
								},
								["g"] = {	
									i(118687),	-- Grizzled Wolfskin Cloak
								},
							}, 
							{	-- Guk
								["npcID"] = 98200,	-- Guk
								["questID"] = 40074,
								["coords"] = {
									{ 23.78, 38.59, 550 },
								},
								["isDaily"] = true,								
								["g"] = {
									i(129218),	-- Glittering Arcane Crystal (Empyreal Manafiend Pet)
								},
							},
							{	-- Highmaul Sledge
								["objectID"] = 233611,	-- Highmaul Sledge
								["questID"] = 36039,
								["coords"] = {
									{ 67.38, 49.08, 550 },
								},
								["g"] = {
									i(118252),	-- Trophy Loop of the Highmaul
								},
							},
							{	-- Hunter Blacktooth
								["npcID"] = 83603,	-- Hunter Blacktooth
								["questID"] = 35923,
								["coords"] = {
									{ 80.6, 30.6, 550 },
								},
								["g"] = {	
									i(118245),	-- Hunter Blacktooth's Ribcruncher
								},
							}, 
							{	-- Hyperious	**Coords uncofirmed, relied on wowhead**
								["npcID"] = 78161,	-- Hyperious
								["coords"] = {
									{ 87.0, 54.8, 550 },
								},
								["description"] = "Use the 3 braziers around the pit to summon him.",
								["g"] = {
									i(116799),	-- Smoldering Heart of Hyperious
								},
							},
							{	-- Karosh Blackwind	**Coords uncofirmed, relied on wowhead**
								["npcID"] = 86959,	-- Karosh Blackwind
								["questID"] = 37399,
								["isDaily"] = true,								
								["coords"] = {
									{ 45.8, 34.8, 550 },
								},
								["g"] = {
									{	-- Karosh Blackwind
										["achievementID"] = 9541,	-- The Song of Silence (Mok'gol Outpost)
										["criteriaID"] = 1,			-- Karosh Blackwind
									},
									{	-- Leggings of Howling Winds
										["itemID"] = 119355,	-- Leggings of Howling Winds
									},
								},
							},
							{	-- Krahl Deadeye
								["npcID"] = 87239,	-- Krahl Deadeye
								["questID"] = 37473,
								["isDaily"] = true,
								["coords"] = {
									{ 42.6, 36.2, 550 },
								},
								["description"] = "Obtain random area drop Secret Meeting Details and then blow the Signal Horn at coords 42, 36 to summon Gortag and Krahl.",
								["g"] = {
									{	-- Krahl Deadeye
										["achievementID"] = 9541,	-- The Song of Silence (Mok'gol Outpost)
										["criteriaID"] = 3,			-- Krahl Deadeye
									},
								},
							},					
							{	-- Krud the Eviscerator	**Coords uncofirmed, relied on wowhead**
								["npcID"] = 88210,	-- Krud the Eviscerator
								["questID"] = 37398,
								["coords"] = {
									{ 58.2, 12.0, 550 },
								},
								["isDaily"] = true,								
								["description"] = "|cff66ccffKill 15 mobs near him to make him attackable.|r",
								["g"] = {
									i(119384),	-- Krud's Girthy Girdle
								},
							}, 
							{	-- Lost Pendant
								["objectID"] = 233651,	-- Lost Pendant
								["questID"] = 36082,
								["coords"] = {
									{ 61.8, 57.5, 550 },
								},
								["groups"] = {
									i(116687),	-- Oshu'gun Amulet
								},
							},
							{	-- Luk'hok	**Coords uncofirmed, relied on wowhead**
								["npcID"] = 50981,	-- Luk'hok
								["g"] = {
									i(116661),		-- Mottled Meadowstomper Mount
								},
								["coords"] = {
									{ 84.6, 64.0, 550 },
									{ 79.6, 56.6, 550 },
									{ 72.8, 53.4, 550 },
									{ 67.6, 42.6, 550 },
									{ 76.0, 30.6, 550 },
								},
							}, 
							{	-- Malroc Stonesunder
								["npcID"] = 83643,	-- Malroc Stonesunder
								["questID"] = 35932,
								["coords"] = {
									{ 81.2, 60.0, 550 },
								},
								["g"] = {
									i(116796),	-- Malroc's Staff of Command
								},
							}, 
							{	-- Mr. Pinchy Sr.
								["npcID"] = 84435,	-- Mr. Pinchy Sr.
								["questID"] = 36229,
								["coords"] = {
									{ 45.6, 15.2, 550 },
								},
								["g"] = {
									i(118690),	-- Empty Crawdad Trap
								},
							},
							{	-- Nakk the Thunderer	**Coords uncofirmed, relied on wowhead**
								["npcID"] = 50990,	-- Nakk the Thunderer
								["g"] = {
									i(116659),		-- Bloodhoof Bull Mount
								},
								["coords"] = {
									{ 62.8, 15.8, 550 },
									{ 65.0, 20.2, 550 },
									{ 60.8, 31.8, 550 },
									{ 55.8, 35.2, 550 },
									{ 50.2, 34.8, 550 },
								},
							}, 
							{	-- Netherspawn
								["npcID"] = 83401,	-- Netherspawn
								["questID"] = 35865,
								["coords"] = {
									{ 47.6, 70.39, 550 },
								},
								["g"] = {
									i(116815),	-- Netherspawn, Spawn of Netherspawn Pet
								},
							}, 
							{	-- Ogre Beads
								["objectID"] = 233618,	-- Ogre Beads
								["questID"] = 36049,
								["coords"] = {
									{ 80.9, 79.8, 550 },
								},
								["g"] = {
									i(118255),	-- Trophy Ring of Gordal
								},
							},
							{	-- Ophiis
								["npcID"] = 83409,	-- Ophiis
								["questID"] = 35875,
								["coords"] = {
									{ 39.0, 50.0, 550 },
									{ 42.0, 50.2, 550 },
									{ 45.4, 47.4, 550 },
								},
								["g"] = {	
									i(116765),	-- Positive Pantaloons
								},
							}, 
							{	-- Outrider Duretha
								["npcID"] = 83680,	-- Outrider Duretha
								["questID"] = 35943,
								["coords"] = {
									{ 61.8, 69.0, 550 },
								},
								["g"] = {	
									i(116800),	-- Duretha's Trail Boots
								},
							}, 
							{	-- Pit Beast	**Coords uncofirmed, relied on wowhead**
								["npcID"] = 88208,	-- Pit Beast
								["questID"] = 37637,
								["coords"] = {
									{ 58.2, 18.4, 550 },
								},
								["isDaily"] = true,							
								["g"] = {	
									i(120317),	-- Pristine Hide of the Pit Beast
								},
							}, 
							{	-- Pit Slayer	**Coords uncofirmed, relied on wowhead**
								["npcID"] = 87846,	-- Pit Slayer
								["questID"] = 37397,
								["isDaily"] = true,
								["coords"] = {
									{ 39.6, 14.6, 550 },
								},
								["description"] = "Must be on Assault on the Broken Precipice, click on blue crystals, click on trophy as an ogre.",
								["g"] = {
									{	-- Pit Slayer
										["achievementID"] = 9571,	-- Broke Back Precipice (Broken Precipice)
										["criteriaID"] = 3,			-- Pit Slayer
									},
									{	-- Pit-Slayer's Magmastone
										["itemID"] = 119389,	-- Pit-Slayer's Magmastone
									},
								},
							},
							{	-- Polished Saberon Skull
								["objectID"] = 233593,	-- Polished Saberon Skull
								["questID"] = 36035,
								["coords"] = {
									{ 72.7, 61.0, 550 },
								},
								["g"] = {
									i(118254),	-- Trophy Signet of the Sabermaw
								},
							},
							{	-- Pokkar's Thirteenth Axe
								["objectID"] = 233561,	-- Pokkar's Thirteenth Axe
								["questID"] = 36021,
								["coords"] = {
									{ 58.3, 59.5, 550 },
								},
								["g"] = {
									i(116688),	-- Pokhar's Thirteenth Axe
								},
							},					
							{	-- Pugg
								["npcID"] = 98199,	-- Pugg
								["questID"] = 40073,
								["coords"] = {
									{ 28.17, 29.67, 550 },
								},
								["isDaily"] = true,							
								["g"] = {
									i(129217),	-- Warm Arcane Crystal (Empowered Manafiend Pet)
								},
							}, 
							{	-- Redclaw the Feral
								["npcID"] = 82755,	-- Redclaw the Feral
								["questID"] = 35712,
								["coords"] = {
									{ 73.6, 57.8, 550 },
								},
								["g"] = {	
									i(118243),	-- Redclaw's Gutripper
								},
							}, 
							{	-- Ru'klaa
								["npcID"] = 83526,	-- Ru'klaa
								["questID"] = 35900,
								["coords"] = {
									{ 58.0, 83.9, 550 },
								},
								["g"] = {	
									i(118688),	-- Carapace Shell Shoulders
								},
							}, 
							{	-- Rukdug
								["npcID"] = 98198,	-- Rukdug
								["questID"] = 40075,
								["coords"] = {
									{ 25.94, 34.74, 550 },
								},
								["isDaily"] = true,							
								["g"] = {
									i(129216),	-- Vibrating Arcane Crystal (Energized Manafiend Pet)
								},
							}, 
							{	-- Saberon Stash
								["objectID"] = 233697,	-- Saberon Stash
								["questID"] = 36102,
								["coords"] = {
									{ 75.2, 64.9, 550 },
								},
								["g"] = {
									-- Weapons
									i(116627),	-- Howling Axe
									i(116629),	-- Howling Bow
									i(116628),	-- Howling Broadaxe
									i(116630),	-- Howling Crossbow
									i(116631),	-- Howling Dagger
									i(116642),	-- Howling Greatsword
									i(116634),	-- Howling Gun
									i(116637),	-- Howling Hammer
									i(116633),	-- Howling Knuckles
									i(116635),	-- Howling Mace
									i(116638),	-- Howling Spear
									i(116640),	-- Howling Staff
									i(116639),	-- Howling Staff
									i(116641),	-- Howling Sword
									i(116724),	-- Oshu'gun Basin
									i(116636),	-- Oshu'gun Scepter
									i(116632),	-- Oshu'gun Spellblade
									i(116643),	-- Oshu'gun Wand
									i(106727),	-- Stonecrag Shield
									-- Head
									i(106738),	-- Dark Star Cowl
									i(106733),	-- Meadowstomper Hood
									i(106749),	-- Packrunner Helm
									i(106723),	-- Stonecrag Helm
									-- Neck
									i(116687),	-- Oshu'gun Amulet
									-- Shoulders
									i(106743),	-- Dark Star Shoulderpads
									i(106735),	-- Meadowstomper Shoulders
									i(106751),	-- Packrunner Spaulders
									i(106725),	-- Stonecrag Pauldrons
									-- Back
									i(116723),	-- Oshu'gun Cloak
									-- Chest
									i(106741),	-- Dark Star Robe
									i(106734),	-- Meadowstomper Jerkin
									i(106752),	-- Packrunner Vest
									i(106720),	-- Stonecrag Breastplate
									-- Wrist
									i(106744),	-- Dark Star Wristwraps
									i(106729),	-- Meadowstomper Bindings
									i(106746),	-- Packrunner Bracers
									i(106728),	-- Stonecrag Vambraces
									-- Hands
									i(106739),	-- Dark Star Handwraps
									i(106732),	-- Meadowstomper Gloves
									i(106747),	-- Packrunner Gauntlets
									i(106721),	-- Stonecrag Gauntlets
									-- Waist
									i(106737),	-- Dark Star Cord
									i(106736),	-- Meadowstomper Waistband
									i(106745),	-- Packrunner Belt
									i(106722),	-- Stonecrag Girdle
									-- Legs
									i(106740),	-- Dark Star Leggings							
									i(106731),	-- Meadowstomper Britches
									i(106750),	-- Packrunner Legguards
									i(106724),	-- Stonecrag Legplates
									-- Feet
									i(106742),	-- Dark Star Sandals
									i(106730),	-- Meadowstomper Boots
									i(106748),	-- Packrunner Greaves
									i(106726),	-- Stonecrag Sabatons
									-- Finger
									i(116725),	-- Oshu'gun Ring
									i(116920, {	-- True Steel Lockbox
										-- Weapons
										i(116721),	-- Ancestral Branch
										i(116619),	-- Ancestral Scepter
										i(116626),	-- Ancestral Wand
										i(116613),	-- Gorian Crossbow
										i(116614),	-- Gorian Dagger
										i(116625),	-- Gorian Greatsword
										i(116617),	-- Gorian Gun
										i(116620),	-- Gorian Hammer
										i(116618),	-- Gorian Mace
										i(116621),	-- Gorian Spear
										i(116622),	-- Gorian Staff
										i(116627),	-- Howling Axe
										i(116634),	-- Howling Gun
										i(116633),	-- Howling Knuckles
										i(116640),	-- Howling Staff
										i(116632),	-- Oshu'gun Spellblade
										i(116643),	-- Oshu'gun Wand
										-- Head
										i(106690),	-- Ironfist Helm
										i(106723),	-- Stonecrag Helm
										-- Shoulders
										i(106692),	-- Ironfist Pauldrons
										i(106735),	-- Meadowstomper Shoulders
										i(106702),	-- Sabermaw Shoulders
										-- Chest
										i(106687),	-- Ironfist Breastplate
										i(106701),	-- Sabermaw Jerkin
										i(106720),	-- Stonecrag Breastplate
										i(106708),	-- Voidwrap Robe
										-- Wrist
										i(106729),	-- Meadowstomper Bindings
										i(106713),	-- Sunspring Bracers							
										i(106711),	-- Voidwrap Wristwraps
										-- Hands
										i(106747),	-- Packrunner Gauntlets
										i(106699),	-- Sabermaw Gloves
										i(106714),	-- Sunspring Gauntlets
										i(106706),	-- Voidwrap Handwraps
										-- Waist
										i(106722),	-- Stonecrag Girdle
										i(106712),	-- Sunspring Belt
										i(106704),	-- Voidwrap Cord
										-- Legs
										i(106740),	-- Dark Star Leggings
										i(106691),	-- Ironfist Legplates
										i(106717),	-- Sunspring Legguards
										-- Feet
										i(106697),	-- Sabermaw Boots
										i(106726),	-- Stonecrag Sabatons
									}),
								},
							},
							{	-- Scout Pokhar
								["npcID"] = 83634,	-- Scout Pokhar
								["questID"] = 35931,
								["coords"] = {
									{ 54.8, 61.2, 550 },
								},
								["g"] = {	
									i(116797),	-- Pokhar's Eighth Axe
								},
							}, 
							{	-- Sean Whitesea
								["npcID"] = 83542,	-- Sean Whitesea
								["questID"] = 35912,
								["coords"] = {
									{ 60.9, 47.78, 550 },
								},
								["description"] = "Spawns after abandoned chest is looted.",
								["g"] = {
									i(116834),	-- Whitesea's Waistwrap
								},
							}, 
							{	-- Soulfang
								["npcID"] = 80057,	-- Soulfang
								["questID"] = 36128,
								["coords"] = {
									{ 75.6, 65.2, 550 },
								},
								["g"] = {	
									i(116806),	-- Soul Fang
								},
							}, 
							{	-- Telaar Defender Shield
								["objectID"] = 233613,	-- Telaar Defender Shield
								["questID"] = 36046,
								["coords"] = {
									{ 64.68, 65.82, 550 },
								},
								["g"] = {
									i(118253),	-- Trophy Band of Telaar
								},
							},
							{	-- Tura'aka
								["npcID"] = 83591,	-- Tura'aka
								["questID"] = 35920,
								["coords"] = {
									{ 65.0, 39.2, 550 },
								},
								["g"] = {	
									i(116814),	-- Tura'aka's Clipped Wing
								},
							}, 
							{	-- Vileclaw
								["npcID"] = 88951,	-- Vileclaw
								["coords"] = {
									{ 37.21, 39.01, 550 },
								},
								["g"] = {
									{	-- Vileclaw's Claw
										["itemID"] = 120172,	-- Vileclaw's Claw
									},
								},
							},
							{	-- Void-Infused Crystal	**Coords uncofirmed, relied on wowhead**
								["objectID"] = 232590,	-- Void-Infused Crystal
								["questID"] = 35579,	
								["coords"] = {
									{ 50., 66.5, 550 },
								},
								["g"] = {
									i(118264),	-- Serrated Void Crystal
								},
							},					
							{	-- Warleader Tome
								["npcID"] = 81330,	-- Warleader Tome
								["questID"] = 37546,
								["isDaily"] = true,
								["coords"] = {
									{ 81.35, 60.34, 550 },
								},
								["description"] = "Spawns at Anguish Point and wanders the zone starting south along the road to pass Grommashar.",
								["g"] = {
									i(120276),	-- Outrider's Bridle Chain Toy
								},
							},
							{	-- Warmaster Blugthol
								["npcID"] = 79024,	-- Warmaster Blugthol
								["questID"] = 34645,
								["coords"] = {
									{ 82.6, 76.2, 550 },
								},
								["g"] = {	
									i(116805),	-- Blug'thol's Bloody Bracers
								},
							}, 
							{	-- Warsong Helm
								["objectID"] = 233645,	-- Warsong Helm
								["questID"] = 36073,
								["coords"] = {
									{ 52.5, 44.5, 550 },
								},
								["g"] = {
									i(118250),	-- Riverwashed Warsong Helm
								},
							},						
							{	-- Windcaller Korast
								["npcID"] = 83428,	-- Windcaller Korast
								["questID"] = 35877,
								["coords"] = {
									{ 70.6, 29.6, 550 },
								},
								["g"] = {	
									i(116808),	-- Whirlwind's Harvest
								},
							}, 
						},
					},
				},
			},
		},
	},
};
