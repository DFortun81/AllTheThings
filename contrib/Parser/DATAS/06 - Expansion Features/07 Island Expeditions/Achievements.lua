-------------------------------------------------------------------
--      E X P A N S I O N   F E A T U R E S    M O D U L E       --
-------------------------------------------------------------------
-- TO:DO -- Break the file down in parts later. Lucetia
-- Add icons to each Item Set header
_.ExpansionFeatures =
{
	n(-3338, {	-- Island Expeditions
		["groups"] = {
			n(-4, {		-- Achievements
				["groups"] = {
					{	-- Expedition Leader
						["achievementID"] = 13134,	-- Expedition Leader
						["g"] = {
							{	-- <Name> Expedition Leader
								["style"] = 2,
								["titleID"] = 382,	-- <Name> Expedition Leader
							},
							{	-- Azerite Admiral	
								["criteriaID"] = 2,	-- Azerite Admiral
							},		
							{	-- Expert Expeditioner	
								["criteriaID"] = 9,	-- Expert Expeditioner
							},		
							{	-- Give Me The Energy	
								["criteriaID"] = 3,	-- Give Me The Energy
							},		
							{	-- Helping Hand	
								["criteriaID"] = 7,	-- Helping Hand
							},		
							{	-- I'm Here for the Pets	
								["criteriaID"] = 6,	-- I'm Here for the Pets
							},		
							{	-- Island Conqueror	
								["criteriaID"] = 1,	-- Island Conqueror
							},		
							{	-- Island Slayer	
								["criteriaID"] = 11,	-- Island Slayer
							},		
							{	-- Metal Detector	
								["criteriaID"] = 5,	-- Metal Detector
							},		
							{	-- Notorious	
								["criteriaID"] = 8,	-- Notorious
							},		
							{	-- Team Deathmatch	
								["criteriaID"] = 10,	-- Team Deathmatch
							},		
							{	-- Tell Me A Tale	
								["criteriaID"] = 4,	-- Tell Me A Tale
							},	
						},
					},
					{	-- Achipelago Explorer
						["achievementID"] = 13142,	-- Achipelago Explorer
					},
					{	-- Azerite Admiral
						["achievementID"] = 13129,	-- Azerite Admiral
					},
					{	-- Bayside Brawler
						["achievementID"] = 12597,	-- Bayside Brawler
					},
					{	-- Competent Captain [Note: Not attached to Expert Expeditioner since it was a different difficulty and didn't make sense
						["achievementID"] = 12594,	-- Competent Captain
					},
					{	-- Expert Expeditioner [Note: Not attached to Expert Expeditioner since it was a different difficulty and didn't make sense
						["achievementID"] = 12595,	-- Expert Expeditioner
					},
					{	-- Give Me The Energy
						["achievementID"] = 13126,	-- Give Me The Energy
					},
					{	-- Helping Hand
						["achievementID"] = 13132,	-- Helping Hand
						["g"] = {
							{	-- Aerin Skyhammer 	
								["criteriaID"] = 3,	-- Aerin Skyhammer 
							},		
							{	-- Apprentice Karyn 	
								["criteriaID"] = 11,	-- Apprentice Karyn 
							},		
							{	-- Arwan Beastheart 	
								["criteriaID"] = 25,	-- Arwan Beastheart 
							},		
							{	-- Brunold 	
								["criteriaID"] = 2,	-- Brunold 
							},		
							{	-- Burgle 	
								["criteriaID"] = 16,	-- Burgle 
							},		
							{	-- Flickerwick 	
								["criteriaID"] = 15,	-- Flickerwick 
							},		
							{	-- Gimzy Trickelbank 	
								["criteriaID"] = 8,	-- Gimzy Trickelbank 
							},		
							{	-- Glint-Eyed Frankie 	
								["criteriaID"] = 9,	-- Glint-Eyed Frankie 
							},		
							{	-- Gregg 	
								["criteriaID"] = 18,	-- Gregg 
							},		
							{	-- Grubby Beard 	
								["criteriaID"] = 12,	-- Grubby Beard 
							},		
							{	-- Longpaws 	
								["criteriaID"] = 21,	-- Longpaws 
							},		
							{	-- Manape 	
								["criteriaID"] = 24,	-- Manape 
							},		
							{	-- Nizhoni 	
								["criteriaID"] = 22,	-- Nizhoni 
							},		
							{	-- Old Li 	
								["criteriaID"] = 19,	-- Old Li 
							},		
							{	-- Royston P. Crutchley III 	
								["criteriaID"] = 5,	-- Royston P. Crutchley III 
							},		
							{	-- Spooner the Scalawag 	
								["criteriaID"] = 4,	-- Spooner the Scalawag 
							},		
							{	-- Sylvester 	
								["criteriaID"] = 13,	-- Sylvester 
							},		
							{	-- Taz'anga 	
								["criteriaID"] = 23,	-- Taz'anga 
							},		
							{	-- Tinny 	
								["criteriaID"] = 6,	-- Tinny 
							},		
							{	-- Tweets Lightsprocket 	
								["criteriaID"] = 7,	-- Tweets Lightsprocket 
							},		
							{	-- Two-Socks 	
								["criteriaID"] = 10,	-- Two-Socks 
							},		
							{	-- Valero 	
								["criteriaID"] = 1,	-- Valero 
							},		
							{	-- Vizio the Cartographer 	
								["criteriaID"] = 17,	-- Vizio the Cartographer 
							},		
							{	-- Waterspeaker Deshi 	
								["criteriaID"] = 14,	-- Waterspeaker Deshi 
							},		
							{	-- Yuke 	
								["criteriaID"] = 20,	-- Yuke 
							},		
						},
					},
					{	-- Heroic: Dread Chain	
						["achievementID"] = 13096,	-- Heroic: Dread Chain
					},		
					{	-- Heroic: Molten Cay	
						["achievementID"] = 13100,	-- Heroic: Molten Cay
					},		
					{	-- Heroic: Rotting Mire	
						["achievementID"] = 13104,	-- Heroic: Rotting Mire
					},		
					{	-- Heroic: Skitttering Hollow	
						["achievementID"] = 13108,	-- Heroic: Skitttering Hollow
					},		
					{	-- Heroic: Un'gol Ruins	
						["achievementID"] = 12589,	-- Heroic: Un'gol Ruins
					},		
					{	-- Heroic: Verdant Wilds	
						["achievementID"] = 13112,	-- Heroic: Verdant Wilds
					},		
					{	-- Heroic: Whispering Reef	
						["achievementID"] = 13118,	-- Heroic: Whispering Reef
					},		
					{	-- I'm Here for the Pets
						["achievementID"] = 13128,	-- I'm Here for the Pets
						["g"] = {
							{	-- Barnaby	
								["criteriaID"] = 3,	-- Barnaby
							},		
							{	-- Bloodstone Tunneler	
								["criteriaID"] = 22,	-- Bloodstone Tunneler
							},		
							{	-- Captain Nibs	
								["criteriaID"] = 2,	-- Captain Nibs
							},		
							{	-- Coldlight Surfrunner	
								["criteriaID"] = 27,	-- Coldlight Surfrunner
							},		
							{	-- Craghoof Kid	
								["criteriaID"] = 9,	-- Craghoof Kid
							},		
							{	-- Deathsting Scorpid	
								["criteriaID"] = 13,	-- Deathsting Scorpid
							},		
							{	-- False Knucklebump	
								["criteriaID"] = 10,	-- False Knucklebump
							},		
							{	-- Giggling Flame	
								["criteriaID"] = 15,	-- Giggling Flame
							},		
							{	-- Inky	
								["criteriaID"] = 6,	-- Inky
							},		
							{	-- Juvenile Brineshell	
								["criteriaID"] = 25,	-- Juvenile Brineshell
							},		
							{	-- Kindleweb Spiderling	
								["criteriaID"] = 8,	-- Kindleweb Spiderling
							},		
							{	-- Kunchong Hatchling	
								["criteriaID"] = 26,	-- Kunchong Hatchling
							},		
							{	-- Laughing Stonekin	
								["criteriaID"] = 16,	-- Laughing Stonekin
							},		
							{	-- Littlehoof	
								["criteriaID"] = 19,	-- Littlehoof
							},		
							{	-- Mischievous Zephyr	
								["criteriaID"] = 18,	-- Mischievous Zephyr
							},		
							{	-- Muskflank Calfling	
								["criteriaID"] = 24,	-- Muskflank Calfling
							},		
							{	-- Octopode Fry	
								["criteriaID"] = 5,	-- Octopode Fry
							},		
							{	-- Playful Frostkin	
								["criteriaID"] = 17,	-- Playful Frostkin
							},		
							{	-- Poro	
								["criteriaID"] = 4,	-- Poro
							},		
							{	-- Sandshell Chitterer	
								["criteriaID"] = 12,	-- Sandshell Chitterer
							},		
							{	-- Scuttle	
								["criteriaID"] = 1,	-- Scuttle
							},		
							{	-- Snapper	
								["criteriaID"] = 20,	-- Snapper
							},		
							{	-- Snort	
								["criteriaID"] = 23,	-- Snort
							},		
							{	-- Sparkleshell Sandcrawler	
								["criteriaID"] = 7,	-- Sparkleshell Sandcrawler
							},		
							{	-- Sunscale Hatchling	
								["criteriaID"] = 21,	-- Sunscale Hatchling
							},		
							{	-- Thistlebrush Bud	
								["criteriaID"] = 14,	-- Thistlebrush Bud
							},		
							{	-- Tinder Pup	
								["criteriaID"] = 11,	-- Tinder Pup
							},		
							{	-- Voru'kar Leecher	
								["criteriaID"] = 28,	-- Voru'kar Leecher
							},				
						},
					},
					{	-- Island Conqueror
						["achievementID"] = 13122,	-- Island Conqueror
						["g"] = {
							{	-- Mythic: Dread Chain	
								["achievementID"] = 13097,	-- Mythic: Dread Chain
							},		
							{	-- Mythic: Molten Cay	
								["achievementID"] = 13101,	-- Mythic: Molten Cay
							},		
							{	-- Mythic: Rotting Mire	
								["achievementID"] = 13105,	-- Mythic: Rotting Mire
							},		
							{	-- Mythic: Skittering Hollow	
								["achievementID"] = 13109,	-- Mythic: Skittering Hollow
							},		
							{	-- Mythic: Un'gol Ruins	
								["achievementID"] = 12591,	-- Mythic: Un'gol Ruins
							},		
							{	-- Mythic: Verdant Wilds	
								["achievementID"] = 13113,	-- Mythic: Verdant Wilds
							},		
							{	-- Mythic: Whispering Reef	
								["achievementID"] = 13116,	-- Mythic: Whispering Reef
							},		
						},
					},
					{	-- Island Slayer
						["achievementID"] = 13141,	-- Island Slayer
						["g"] = {
							{	-- Bane of the Chain	
								["achievementID"] = 13098,	-- Bane of the Chain
							},		
							{	-- Clash at the Cay	
								["achievementID"] = 13102,	-- Clash at the Cay
							},		
							{	-- Mire Marauder	
								["achievementID"] = 13106,	-- Mire Marauder
							},		
							{	-- Reef Madness	
								["achievementID"] = 13115,	-- Reef Madness
							},		
							{	-- Ruined the Ruins	
								["achievementID"] = 12592,	-- Ruined the Ruins
							},		
							{	-- Skittering Smashed	
								["achievementID"] = 13110,	-- Skittering Smashed
							},		
							{	-- War for the Wilds	
								["achievementID"] = 13114,	-- War for the Wilds
							},		
						},
					},
					{	-- Metal Detector
						["achievementID"] = 13124,	-- Metal Detector
						["g"] = {
							{	-- My Dubs
								["achievementID"] = 13123,	-- My Dubs
							},
						},
					},
					{	-- No Tourist [Note: Not attached to Expert Expeditioner since it was a different difficulty and didn't make sense
						["achievementID"] = 12596,	-- No Tourist
					},
					{	-- Sucker Punch
						["achievementID"] = 13129,	-- Sucker Punch
					},
					{	-- Team Deathmatch [Alliance Version]
						["achievementID"] = 13133,	-- Team Deathmatch [Alliance Version]
						["races"] = ALLIANCE_ONLY,
						["g"] = {
							{	-- Astralite Visara	
								["criteriaID"] = 7,	-- Astralite Visara
							},		
							{	-- Berserker Zar'ri	
								["criteriaID"] = 13,	-- Berserker Zar'ri
							},		
							{	-- Captain Greenbelly	
								["criteriaID"] = 4,	-- Captain Greenbelly
							},		
							{	-- Dorp	
								["criteriaID"] = 5,	-- Dorp
							},		
							{	-- Gazlowe	
								["criteriaID"] = 1,	-- Gazlowe
							},		
							{	-- Lady Sena	
								["criteriaID"] = 2,	-- Lady Sena
							},		
							{	-- Mahna Flamewhisper	
								["criteriaID"] = 16,	-- Mahna Flamewhisper
							},		
							{	-- Moonscythe Pelani	
								["criteriaID"] = 8,	-- Moonscythe Pelani
							},		
							{	-- Phoenix Mage Rhydras	
								["criteriaID"] = 10,	-- Phoenix Mage Rhydras
							},		
							{	-- Phoenix Mage Ryleia	
								["criteriaID"] = 11,	-- Phoenix Mage Ryleia
							},		
							{	-- Rune Scribe Lusaris	
								["criteriaID"] = 9,	-- Rune Scribe Lusaris
							},		
							{	-- Shadow Hunter Ju'loa	
								["criteriaID"] = 14,	-- Shadow Hunter Ju'loa
							},		
							{	-- Skaggit	
								["criteriaID"] = 3,	-- Skaggit
							},		
							{	-- Sneaky Pete	
								["criteriaID"] = 6,	-- Sneaky Pete
							},		
							{	-- Spiritwalker Quura	
								["criteriaID"] = 17,	-- Spiritwalker Quura
							},		
							{	-- Sunbringer Firasi	
								["criteriaID"] = 12,	-- Sunbringer Firasi
							},		
							{	-- Sunwalker Ordel	
								["criteriaID"] = 18,	-- Sunwalker Ordel
							},		
							{	-- Witch Doctor Unbugu	
								["criteriaID"] = 15,	-- Witch Doctor Unbugu
							},		
						},
					},
					{	-- Team Deathmatch [Horde Version]
						["achievementID"] = 13135,	-- Team Deathmatch [Horde Version]
						["races"] = HORDE_ONLY,
						["g"] = {
							{	-- Anchorite Lanna	
								["criteriaID"] = 7,	-- Anchorite Lanna
							},		
							{	-- Archmage Tamuura	
								["criteriaID"] = 8,	-- Archmage Tamuura
							},		
							{	-- Briona the Bloodthirsty	
								["criteriaID"] = 1,	-- Briona the Bloodthirsty
							},		
							{	-- Dizzy Dina	
								["criteriaID"] = 10,	-- Dizzy Dina
							},		
							{	-- Duskrunner Lorinas	
								["criteriaID"] = 13,	-- Duskrunner Lorinas
							},		
							{	-- Fenrae the Cunning	
								["criteriaID"] = 16,	-- Fenrae the Cunning
							},		
							{	-- Frostfencer Seraphi	
								["criteriaID"] = 4,	-- Frostfencer Seraphi
							},		
							{	-- Gunnolf the Ferocious	
								["criteriaID"] = 17,	-- Gunnolf the Ferocious
							},		
							{	-- Raul the Tenacious	
								["criteriaID"] = 18,	-- Raul the Tenacious
							},		
							{	-- Razak Ironsides	
								["criteriaID"] = 12,	-- Razak Ironsides
							},		
							{	-- Riftblade Kelain	
								["criteriaID"] = 14,	-- Riftblade Kelain
							},		
							{	-- Shadeweaver Zarra	
								["criteriaID"] = 15,	-- Shadeweaver Zarra
							},		
							{	-- Squallshaper Auran	
								["criteriaID"] = 5,	-- Squallshaper Auran
							},		
							{	-- Squallshaper Bryson	
								["criteriaID"] = 6,	-- Squallshaper Bryson
							},		
							{	-- Stabby Lottie	
								["criteriaID"] = 2,	-- Stabby Lottie
							},		
							{	-- Tally Zapnabber	
								["criteriaID"] = 11,	-- Tally Zapnabber
							},		
							{	-- Varigg	
								["criteriaID"] = 3,	-- Varigg
							},		
							{	-- Vindicator Baatul	
								["criteriaID"] = 9,	-- Vindicator Baatul
							},		
						},
					},
					{	-- Tell Me A Tale
						["achievementID"] = 13127,	-- Tell Me A Tale
						["g"] = {
							{	-- Ancient Crypt Key	
								["itemID"] = 163470,	-- Ancient Crypt Key
								["questID"] = 53160,	-- Ancient Crypt Key
								-- Note!! Mogue creatures should drop this.  Still experimental 10/13
								["crs"] = {
									139439,	-- Duskbinder Zuun
									139444,	-- Necrolord Zian
									123146,	-- Torn Spirit
									126190,	-- Zian-Ti Brutalizer
									138972,	-- Zian-Ti Cloudbreaker
									126215,	-- Zian-Ti Darkweaver
									126216,	-- Zian-Ti Destroyer
									139464,	-- Zian-Ti Howler
									139533,	-- Zian-Ti Invasion Serpent
									138968,	-- Zian-Ti Ironmaw
									132807,	-- Zian-Ti Quilen
									138971,	-- Zian-Ti Serpent
									126185,	-- Zian-Ti Shadowcaster
									138969,	-- Zian-Ti Spirit
								},
							},		
							{	-- Battered Twilight Scale	
								["itemID"] = 163478,	-- Battered Twilight Scale
								["questID"] = 53171,	-- Battered Twilight Scale
							},		
							{	-- Bloodwake Drinking Horn	
								["itemID"] = 163472,	-- Bloodwake Drinking Horn
								["questID"] = 53163,	-- Bloodwake Drinking Horn
							},		
							{	-- Broken Compass	
								["itemID"] = 163482,	-- Broken Compass
								["questID"] = 53177,	-- Broken Compass
							},		
							{	-- Druidic Fur	
								["itemID"] = 163485,	-- Druidic Fur
								["questID"] = 53181,	-- Druidic Fur
							},		
							{	-- Elemental Core	
								["itemID"] = 163476,	-- Elemental Core
								["questID"] = 53169,	-- Elemental Core
							},		
							{	-- Glittering Scarab	
								["itemID"] = 163481,	-- Glittering Scarab
								["questID"] = 53176,	-- Glittering Scarab
							},		
							{	-- Icetusk Fur Cloak	
								["itemID"] = 163474,	-- Icetusk Fur Cloak
								["questID"] = 53167,	-- Icetusk Fur Cloak
							},		
							{	-- Magical Spyglass	
								["itemID"] = 163475,	-- Magical Spyglass
								["questID"] = 53168,	-- Magical Spyglass
							},		
							{	-- Murloc Eye	
								["itemID"] = 163480,	-- Murloc Eye
								["questID"] = 53174,	-- Murloc Eye
							},		
							{	-- Notched Coin	
								["itemID"] = 163466,	-- Notched Coin
								["questID"] = 53155,	-- Notched Coin
							},		
							{	-- Razorfin Hunting Spear	
								["itemID"] = 163483,	-- Razorfin Hunting Spear
								["questID"] = 53178,	-- Razorfin Hunting Spear
							},		
							{	-- Ring of the Reefs	
								["itemID"] = 163471,	-- Ring of the Reefs
								["questID"] = 53161,	-- Ring of the Reefs
							},		
							{	-- Sonic Stone	
								["itemID"] = 163473,	-- Sonic Stone
								["questID"] = 53164,	-- Sonic Stone
							},		
							{	-- Strangling Root	
								["itemID"] = 163486,	-- Strangling Root
								["questID"] = 53182,	-- Strangling Root
							},		
							{	-- Unscarred Black Scale	
								["itemID"] = 163477,	-- Unscarred Black Scale
								["questID"] = 53170,	-- Unscarred Black Scale
							},		
							{	-- Venture Co. Business Plan	
								["itemID"] = 163479,	-- Venture Co. Business Plan
								["questID"] = 53172,	-- Venture Co. Business Plan
								-- Note!! Venture Co. creatures should drop this.  Still experimental 10/6
								["crs"] = {
									139817,	-- Chief Engineer Zazzy
									139805,	-- Mechanical Guardhound
									139814,	-- Merger Specialist Huzzle
									126205,	-- Profit-O-Matic
									131431,	-- Safety Inspection Bot
									139818,	-- Security Officer Durk
									139813,	-- Senior Producer Gixi
									139809,	-- Venture Acquisitions Specialist
									139806,	-- Venture Demolitions Expert
									131407,	-- Venture Goon
									139804,	-- Venture Inspector
									139810,	-- Venture Middle Manager
									131408,	-- Venture Muscle
									139808,	-- Venture Oaf
									139812,	-- Venture Producer
									139811,	-- Venture Sub-Lead
									139807,	-- Venture Surveyor
								},
							},		
							{	-- Vorus'arak's Carapace	
								["itemID"] = 163484,	-- Vorus'arak's Carapace
								["questID"] = 53180,	-- Vorus'arak's Carapace
							},		
							{	-- Weathered Pamphlet	
								["itemID"] = 163488,	-- Weathered Pamphlet
								["questID"] = 53184,	-- Weathered Pamphlet
							},		
							{	-- Wriggling Mass	
								["itemID"] = 163487,	-- Wriggling Mass
								["questID"] = 53183,	-- Wriggling Mass
							},		
						},
					},
				},
			}),
		},
		["description"] = "|cff66ccffIsland expeditions are a 3-player scenario.  Each factiton will sail across the waters where they will harvest and steal any Azerite they can find from these islands.  Goblins and gnomes have been able to use their new technology to find islands which contain possible amounts of Azerite and will be providing maps for each of their respective factions.  Ships and queuing will take places in Dazar'alor for Horde and Boralus for Alliance.  These unlock at level 116 on your first playthrough and should be open at 110 on your alts once you have finished the introduction line. |r",
		["achievementID"] = 12595, -- Expert Expeditioner
		["lvl"] = 110,
		["crs"] = {
			143968,	--  Expedition Map [Alliance Side]
			143967,	--  Expedition Map [Horde Side]
		},
		-- NOTE: For now we'll attach the Island Maps here until we decide if we want to rearrange them
		["maps"] = {
			1036,	-- Dread Chain
			1033,	-- Rotting Mire
			981,	-- Un'gol Ruins
			1037,	-- Whispering Reef
			1035,	-- Molten Cray
			1032,	-- Skittering Hollow
			1034,	-- Verdant Wilds
		},
	}),
};